#!/usr/bin/env python3
"""
TEST_M2_EQUILIBRIUM: Equilibrium vs. Non-Equilibrium Initial Conditions Test
Ray-based parallel execution of Athena++ filament fragmentation simulations

Purpose: Address theoretical reviewer concern T-M2 — establish whether DTC results
are robust to initial condition choice or contaminated by transient radial collapse
...
STATUS_FILE = Path.cwd() / "status_equilibrium.json"
...
IC_TYPES = CONFIG["ic_types"]

print(f"TEST_M2_EQUILIBRIUM: Initial conditions convergence test")
print(f"  Test points: {len(TEST_POINTS)}")
print(f"  IC types: {len(IC_TYPES)} ({[ic['name'] for ic in IC_TYPES]})")
print(f"  Total simulations: {len(TEST_POINTS) * len(IC_TYPES)}")

def wavelength_for_beta(beta):
    return np.sqrt(1.0 + 2.0 / beta)

def make_input_file(sim, sim_dir):
    """Generate Athena++ input file for equilibrium test."""
    lam = wavelength_for_beta(sim["beta"])
    ic_type = sim["ic_type"]

    # IC-specific parameters
    if ic_type == "profile":
        # DTC standard: profile-based initial conditions
        ic_note = "Profile-based IC: rho(r) = rho_0 / [1 + (r/W)^2]"
    else:  # uniform
        ic_note = "Uniform IC: constant rho, constant B, random perturbations"

    content = f"""# TEST_M2_EQUILIBRIUM: Initial conditions test
# {sim['name']} — IC={ic_type}, f={sim['f']}, beta={sim['beta']}, M={sim['mach']}
    ...
    def run_sim_ray(sim, work_dir_str, athena_bin):
        ...
        try:
            ...
            return {
                ...
                "elapsed_time": time.time() - t0, "ic_type": sim["ic_type"],
            }
    ...
    def main():
        ...
        for ic in IC_TYPES:
            ...
            for pt in TEST_POINTS:
                ...
                sim_name = f"EQUIL_{ic_type}_{pt_id}_f{f}_b{beta}_M{mach}"

                simulations.append({
                    "name": sim_name, "f": f, "beta": beta, "mach": mach,
                    "seed": 42, "ic_type": ic_type,
                })
