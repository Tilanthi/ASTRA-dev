% astra-rasti.tex — ASTRA RASTI Paper V2.1
% Original conversion from PDF text on 2026-04-04; enhanced to V2.0; revised to V2.1
...
%     (Kepler: 2,839 exoplanets, R²=0.9982, multivariate fit with stellar mass;
%      SNe Ia: μ=5.33·log₁₀(z)+24.42, χ²/dof=0.64;
%      Galaxy bimodality: u-g peaks at 1.44 and 1.92, blue fraction decline)
%   - Added confounder analysis subsection (u-band magnitude bias=0.2351)
%   - Added 6 bibliography entries: Buckingham1914, Pearl2009, Zhang2008,
%     Schwarz1978, Brout2022, Strateva2001; cited in text
%   - Strengthened Discussion: genuine contributions vs confirming known science;
%     more specific limitations; total data volume (27,430+ across 9 sources)
%   - Fixed Conclusion: "Three Modes of Evidence" (was "Two Modes of Operation")
%   - Added live deployment summary to Conclusion
%   - Improved statistical reporting (χ²/dof, effect sizes)
%   - Writing quality: stronger topic sentences, reduced passive voice, ABT flow
%
% V2.0 enhancements:
%   - Strengthened introduction with ABT narrative structure
%   - Restored detailed TC3 sub-bullets (ASTRA identified / Literature / Significance)
%   - Added Live System Validation section (Kepler, SNe Ia, galaxy bimodality, HR diagram, causal direction)
...
\astra chains dimensional analysis via the Buckingham $\pi$ theorem \citep{Buckingham1914}, structural causal models, Bayesian evidence computation, and multi-wavelength data fusion into coherent, physics-aware workflows.
We validate the framework through six controlled test cases drawn from nine independent data sources spanning more than 27\,430 data points---Herschel filament scaling relations, Chandra Deep Field South multi-wavelength cross-matching, SDSS galaxy property analysis, Gaia stellar causal inference, Bayesian model comparison, and a discovery-mode demonstration on synthetic data with known ground truth.
In the discovery test, \astra operates in ``knowledge isolation mode'' to correctly identify the causal drivers of star formation (gravitational instability, magnetic suppression, turbulent support), distinguish column density as a proxy rather than direct cause, and generate testable predictions validated through intervention analysis.
Beyond these six controlled tests, five live deployments on archival data independently recover established physics without prior domain guidance: Kepler's third law from 2\,839 exoplanets ($R^{2}=0.9982$; multivariate fit $\log P = 1.497\,\log a - 0.474\,\log M_{\star}$, capturing stellar-mass dependence), the observational signature of dark energy from 1\,701 Type~Ia supernovae ($\mu = 5.33\,\log_{10}z + 24.42$; $\chi^{2}/\mathrm{dof} = 0.64$; \citealt{Brout2022} Pantheon+ compilation), and galaxy colour bimodality from 2\,000 SDSS galaxies ($u{-}g$ peaks at 1.44 and 1.92, with the blue fraction declining from ${\sim}50$\,per\,cent to ${\sim}10$\,per\,cent with increasing stellar mass; consistent with \citealt{Strateva2001}).
We discuss capabilities and limitations, emphasising \astra's role as a tool to assist astronomers rather than replace domain expertise.
...
Modern astronomical surveys now generate petabytes of multi-wavelength data per year, and a rich ecosystem of computational tools---from \textsc{scikit-learn} classifiers to nested-sampling codes such as \textsc{dynesty}, from the \pc algorithm for causal discovery \citep{Spirtes2000} to frontier large language models with code execution---gives astronomers unprecedented analytical power.
...
Section~\ref{sec:live} presents five live deployments on archival data that independently recover established physics.
...
\begin{enumerate}
  \item[(i)] \textbf{Dimensional analysis:} Identify dimensionless parameters using the Buckingham $\pi$ theorem \citep{Buckingham1914}
...
The correlation coefficient is $r = 0.9883$ ($p < 10^{-18}$), providing overwhelming evidence for a scaling relation.
The effect size ($r^{2} = 0.977$) indicates that mass-to-length ratio explains 97.7~per~cent of the variance in velocity dispersion.
...
\begin{enumerate}
  \item[(i)] \textbf{Mass--metallicity relation:}
  \begin{itemize}
    \item \emph{\astra identified:} Positive correlation between stellar mass and gas-phase metallicity.
    \item \emph{Literature:} Established relationship for star-forming galaxies \citep{Tremonti2004}.
    \item \emph{Significance:} Demonstrates pattern recognition capability---\astra recovers this fundamental scaling relation without being explicitly programmed to search for it.
  \end{itemize}

  \item[(ii)] \textbf{Environmental quenching:}
  \begin{itemize}
    \item \emph{\astra identified:} Anti-correlation between local galaxy density and star formation rate in high-density regions.
    \item \emph{Literature:} Well-established environmental effects on star formation \citep{Dressler1980,Peng2010}.
    \item \emph{Significance:} Correctly identifies density-dependent star formation suppression, a key result in galaxy evolution.
  \end{itemize}

  \item[(iii)] \textbf{AGN signatures in low-mass galaxies:}
  \begin{itemize}
    \item \emph{\astra identified:} Small subset of low-mass galaxies with elevated X-ray emission.
    \item \emph{Literature:} Active area of research; AGN feedback in low-mass galaxies remains debated.
    \item \emph{Significance:} Pattern recognition identifies outlier subpopulations that may warrant targeted follow-up.
  \end{itemize}

  \item[(iv)] \textbf{Merger rate evolution:}
  \begin{itemize}
    \item \emph{\astra identified:} Merger indicators increase with redshift within the sample range ($z < 0.2$).
    \item \emph{Literature:} Well-established merger rate evolution \citep{Lotz2011}.
    \item \emph{Significance:} Recovers known evolutionary trend from local data without prior knowledge of the expected relationship.
  \end{itemize}

  \item[(v)] \textbf{Halo mass correlations:}
  \begin{itemize}
    \item \emph{\astra identified:} Stellar mass correlates with environmental metrics consistent with dark matter halo mass.
    \item \emph{Literature:} Underpins the halo model framework \citep{MoWhite1996}.
    \item \emph{Significance:} Identifies the galaxy--halo connection, a cornerstone of modern galaxy formation theory.
  \end{itemize}
\end{enumerate}
...
The high correlation coefficient ($r = 0.988$, $p < 10^{-18}$) reflects the clear scaling relation but also raises questions about sample representativeness and potential selection effects.
...
The first five test cases demonstrate \astra's ability to correctly recover known astrophysical results using real observational data.
...
Causal inference methods---such as the \fci algorithm \citep{Zhang2008} and do-calculus \citep{Pearl2009}---are required to identify which relationships are genuinely causal versus confounded.
...
The \fci (Fast Causal Inference) algorithm \citep{Zhang2008} is applied to identify causal relationships while explicitly accounting for latent (unmeasured) confounders.
...
\astra simulates causal interventions using do-calculus \citep{Pearl2009} (e.g., ``what happens to $\sfr$ if we vary Jeans mass while holding other variables fixed?'') to distinguish correlation from causation and validate causal claims against ground truth.
...
In knowledge isolation mode (no prior knowledge of what patterns to expect), \astra discovered five significant correlations (all $p$-values well below $10^{-6}$, indicating robust detections):
\begin{enumerate}
  \item[(i)] Column density -- $\sfr$: $r = 0.537$, $p = 1.14 \times 10^{-38}$ ($r^{2} = 0.29$)
  \item[(ii)] Column density -- Jeans mass: $r = 0.840$, $p = 1.25 \times 10^{-134}$ ($r^{2} = 0.71$)
  \item[(iii)] Jeans mass -- $\sfr$: $r = 0.598$, $p = 6.91 \times 10^{-50}$ ($r^{2} = 0.36$)
  \item[(iv)] Magnetic field -- $\sfr$: $r = 0.477$, $p = 8.72 \times 10^{-30}$ ($r^{2} = 0.23$)
  \item[(v)] Virial parameter -- $\sfr$: $r = 0.225$, $p = 3.78 \times 10^{-7}$ ($r^{2} = 0.05$)
\end{enumerate}
...
The effect sizes range from small ($r^{2}=0.05$ for virial parameter) to large ($r^{2}=0.71$ for column density--Jeans mass), reflecting the different strengths of the embedded relationships.

\subsubsection{Phase 2: Causal Structure Discovery}
\label{sec:tc6:phase2}

The \fci algorithm \citep{Zhang2008} produced a Partial Ancestral Graph with 5~edges, all marked as ``uncertain'' (circle--circle notation).
...
\begin{itemize}
  \item Jeans mass discovered: \textbf{Yes} ($r = 0.598$, $p = 6.91 \times 10^{-50}$)
  \item Magnetic field discovered: \textbf{Yes} ($r = 0.477$, $p = 8.72 \times 10^{-30}$)
  \item Virial parameter discovered: \textbf{Yes} ($r = 0.225$, $p = 3.78 \times 10^{-7}$)
...
(1)~Jeans mass $\rightarrow$ $\sfr$ (strongest effect, $r^{2}=0.36$);
(2)~Magnetic field $\dashv$ $\sfr$ (suppression, $r^{2}=0.23$);
(3)~Virial parameter $\rightarrow$ $\sfr$ (weak modulation, $r^{2}=0.05$);
...
Across these five deployments, the system processed more than 12\,000 individual objects from four independent archives (NASA Exoplanet Archive, Pantheon+, SDSS, Gaia EDR3).
In each case below, \astra operated without prior guidance on what relationships to seek; the system ingested the data, autonomously identified patterns, applied causal and physical reasoning, and returned interpretations that we compare with established results.
...
The automated detection of this confounder demonstrates \astra's ability to identify variables that could bias scientific conclusions if left uncontrolled, a capability that goes beyond simple correlation analysis.
...
These five live-data results, obtained without manual tuning or domain-specific guidance, demonstrate that \astra generalises beyond its controlled test cases.
The recovered relationships span classical mechanics (Kepler's law), cosmology (dark energy), galaxy evolution (colour bimodality), stellar astrophysics (HR diagram), and causal inference (redshift--colour direction), collectively exercising the full breadth of the framework's analytical capabilities.
...
The six controlled test cases and five live deployments in this paper collectively analyse more than 27\,430 data points drawn from nine independent sources (Herschel, Chandra, SDSS, Gaia, NASA Exoplanet Archive, Pantheon+, synthetic molecular clouds, MPA-JHU value-added catalogue, and Gaia EDR3).
Rather than repeating the individual results, we reflect here on what these collective demonstrations reveal about \astra's strengths and limitations as an integrated tool.
...
\begin{enumerate}
  ...
  \item[(iii)] \textbf{Live deployment} (Section~\ref{sec:live}): \astra operates on large archival datasets without manual guidance, independently recovering fundamental results spanning classical mechanics, cosmology, galaxy evolution, stellar astrophysics, and causal inference.
\end{enumerate}
...
The live validation results (Section~\ref{sec:live}), while impressive for a fully automated system, recover well-established physics---Kepler's third law, the Hubble diagram, galaxy colour bimodality, the HR main sequence.
These results should not be over-interpreted as novel discoveries; their value lies in demonstrating that \astra's integrated pipeline produces correct, quantitatively precise results on real data without human intervention.
That an automated system can ingest raw archival data and, without being told what to seek, arrive at $R^{2}=0.9982$ for Kepler's law or $\chi^{2}/\mathrm{dof}=0.64$ for the Hubble diagram is a non-trivial engineering and methodological achievement, even though the underlying physics is textbook material.

\astra's genuine scientific contribution lies in three areas.
First, the \emph{integration itself}: by chaining dimensional analysis, causal inference, and Bayesian model selection into a single workflow, \astra eliminates inconsistencies that arise when astronomers manually combine outputs from separate tools---particularly in uncertainty propagation and physical validation.
...
\begin{itemize}
  \item \textbf{Sample sizes:} The controlled test cases use 24--1000 objects, which are small for production analyses. The live deployments reach larger scales (up to 4\,984 stars), but scaling to survey-scale datasets ($10^{6}$--$10^{9}$ objects) requires further validation of computational performance and statistical robustness.
  ...
  \item \textbf{Live validation scope:} The live deployment results, while quantitatively precise, recover well-established physics and should not be over-interpreted as evidence of discovery capability. They validate the pipeline's correctness, not its ability to find genuinely new science.
\end{itemize}
...
We have presented \astra (Autonomous System for Scientific Discovery in Astrophysics), an integrated framework for physics-aware astrophysical analysis that processes data from nine independent sources spanning more than 27\,430 data points.
Through six controlled test cases and five live deployments, we have demonstrated \astra's technical capabilities while acknowledging the limitations of these demonstration cases.
...
\textbf{Live Deployment} (Section~\ref{sec:live})
\begin{itemize}
  \item Recovered Kepler's third law from 2\,839 exoplanets ($R^{2}=0.9982$, multivariate fit capturing stellar-mass dependence)
  \item Recovered dark energy signature from 1\,701 SNe~Ia ($\chi^{2}/\mathrm{dof}=0.64$)
  \item Detected galaxy colour bimodality from 2\,000 SDSS galaxies ($u{-}g$ peaks at 1.44 and 1.92; blue fraction declining with stellar mass)
  \item Recovered HR diagram main sequence from 4\,984 Gaia stars ($r=0.90$)
  \item Correctly inferred causal direction of redshift $\rightarrow$ colour
  \item Detected $u$-band magnitude as strongest confounder (bias $= 0.2351$)
\end{itemize}

\textbf{Three Modes of Evidence:}
Tests~1--5 validate \astra's ability to correctly recover known astrophysical results using real data (\emph{validation mode}).
Test~6 demonstrates \astra's discovery-mode operation under controlled conditions, including knowledge isolation pattern discovery and intervention analysis to distinguish correlation from causation (\emph{discovery mode}).
The five live deployments confirm that the framework generalises to archival data at scale, independently recovering fundamental physics spanning classical mechanics, cosmology, galaxy evolution, and stellar astrophysics (\emph{live deployment mode}).
