...
m3-L131 predictions: Arch1_true(basis 1) = Prime - Endpoint + Zero = -0.5598
(their wrong-kernel scipy value: -0.6479). Same corrected kernel as run-2:
K(t) = 0.5[ps(s/2) + ps((1-s)/2)] - log(pi), s = -1/2+it.
Runs both Simpson (dps 25) and reports; genome s1/M8 basis 1.
"""
import json
...
def main():
    ...
    phi, eds = make_phi(genomes[key][1])
    ...
    for i in range(N+1):
        ...
        tot += w*kern(t)*u_re(t)
    arch = tot*h/(3*2*pi)
    print(f"Simpson CORRECTED-kernel Arch1 (basis 1, N={N}) = {mp.nstr(arch, 10)}")
    print("m3-L131 identity prediction (Prime-Endpoint+Zero) = -0.5598")
    print("m3 wrong-kernel scipy value                        = -0.6479")
...
if __name__ == '__main__':
    ...
    # basis 2 appended
    import json as _j
    genomes = _j.load(open(GEN))["genomes"]
    key = "s1/M8" if "s1/M8" in genomes else "s1/M64"
    phi, eds = make_phi(genomes[key][2])
    def u_re2(t):
        s = mpc(mpf('-0.5'), t)
        return mpre(quad(lambda x: phi(x)*exp(s*x), eds))
    def kern2(t):
    ...
    for i in range(N+1):
        ...
        w = mpf(4) if i % 2 == 1 else mpf(2)
        if i == 0 or i == N: w = mpf(1)
        tot += w*kern2(t)*u_re2(t)
    arch = tot*h/(3*2*pi)
    print(f"Simpson CORRECTED-kernel Arch1 (basis 2, N={N}) = {mp.nstr(arch, 10)}")
    print("m3-L131 identity prediction (Prime-Endpoint+Zero) = -0.0285")
    print("m3 wrong-kernel scipy value                        = -0.0292")
