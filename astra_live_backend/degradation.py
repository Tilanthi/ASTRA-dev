...
class DegradationDetector:
    ...
    def check_after_cycle(self, discovery_memory, cycle: int) -> Dict:
        ...
        # 5. Domain concentration (astrophysics-aware)
        discoveries = list(discovery_memory.discoveries)
        recent_disc = discoveries[-50:] if len(discoveries) >= 50 else discoveries

        # ASTRA focuses on astrophysics, so we check for OVER-SPECIALIZATION
        # within astrophysics subdomains, not total astrophysics concentration
        domain_counts = Counter(d.domain for d in recent_disc)
        data_source_counts = Counter(d.data_source for d in recent_disc)
        ...
        if total_recent > 0:
            # Check domain concentration (but allow astrophysics dominance)
            for domain, count in domain_counts.items():
                frac = count / total_recent
                domain_concentration[domain] = round(frac, 3)

                # Only flag non-astrophysics domains as problematic (if they appear)
                # OR flag if we're missing astrophysics subdomains
                if domain != "Astrophysics" and frac > 0.5:
                    dominant_domain = domain
                    actions.append("DIVERSIFY_DOMAINS")
                    self._recommendations.append(
                        f"Domain '{domain}' has {frac:.0%} of recent discoveries — "
                        f"refocus on astrophysics subdomains"
                    )

            # Check for over-concentration in single data source (too narrow)
            for source, count in data_source_counts.items():
                frac = count / total_recent
                if frac > 0.7:  # 70%+ from one source = too narrow
                    dominant_source = source
                    actions.append("DIVERSIFY_SOURCES")
                    self._recommendations.append(
                        f"Data source '{source}' has {frac:.0%} of recent {total_recent} "
                        f"discoveries — explore other astrophysical data sources"
                    )
