...
def mass_luminosity_test(masses: np.ndarray, luminosities: np.ndarray,
                         theoretical_exponent: float = 3.5,
                         alpha: float = 0.05) -> StatTestResult:
    """
    Test the theoretical mass-luminosity relation L ∝ M^α against observed data.

    From first principles derivation:
    - Hydrostatic equilibrium: P ∝ GM²/R⁴
    - Radiative diffusion: L ∝ R⁴T⁴/κρ
    - Kramers opacity: κ ∝ ρT^(-3.5)
    - Result: L ∝ M^α where α ≈ 3.5 (intermediate mass stars)

    The test fits log L = α log M + β and compares α to theoretical value.

    Args:
        masses: Stellar masses [M☉]
        luminosities: Stellar luminosities [L☉]
        theoretical_exponent: Expected exponent from theory (default 3.5)
        alpha: Significance level

    Returns:
        StatTestResult with fit parameters and comparison to theory
    """
    ...
    log_L_pred = np.polyval(coeffs, log_M)
    residuals = log_L - log_L_pred
    rss = np.sum(residuals**2)
    tss = np.sum((log_L - np.mean(log_L))**2)
    r_squared = 1 - rss/tss

    # Standard error of the slope
    n = len(masses)
    se_alpha = np.sqrt(rss / (n - 2) / np.sum((log_M - np.mean(log_M))**2))

    # t-test for difference from theoretical exponent
    t_stat = (alpha_obs - theoretical_exponent) / se_alpha
    p_value = 2 * (1 - stats.t.cdf(abs(t_stat), df=n-2))

    passed = p_value > alpha

    details = (
        f"log L = {alpha_obs:.3f} log M + {beta_obs:.3f}, "
        f"R² = {r_squared:.4f}, "
        f"α_theory = {theoretical_exponent}, "
        f"t = {t_stat:.3f}"
    )

    return StatTestResult(
        test_name="Mass-Luminosity Relation",
...
def power_law_fit(x: np.ndarray, y: np.ndarray, alpha: float = 0.05) -> dict:
    """
    Generic power law fit: y = A x^β

    Returns coefficients, uncertainties, and goodness-of-fit.
    """
    # Log transform
    log_x = np.log10(x)
    log_y = np.log10(y)
    ...
    se_A = A * np.sqrt(cov[1, 1]) * np.log(10)
    ...
    n = len(x)
    rmse = np.sqrt(rss / (n - 2))

    return {
        "relation": f"y = {A:.3e} x^{beta:.3f}",
        "A": float(A),
        "A_se": float(se_A),
        "beta": float(beta),
