#!/usr/bin/env python3
"""
Minimal Continuous Discovery Memory Monitor

Imports bootstrap_memory directly to avoid domain loading.
"""
...
# Direct import of bootstrap_memory module
ASTRA_DEV = Path("/Users/gjw255/astrodata/SWARM/ASTRA-dev-main")
bootstrap_path = ASTRA_DEV / "astra_core" / "memory" / "persistent"
...
# Initialize memory
bootstrap = bootstrap_memory.BootstrapMemory()
...
def get_discovery_count():
    ...
    try:
        discoveries = bootstrap.get_memories_by_category(bootstrap_memory.MemoryCategory.CRITICAL_KNOWLEDGE)
...
def store_discovery(statement, r_val, p_val, index):
    ...
    try:
        ...
        memory_item = bootstrap_memory.PersistentMemoryItem(
            id=f"discovery_auto_{index}",
            category=bootstrap_memory.MemoryCategory.CRITICAL_KNOWLEDGE,
            priority=bootstrap_memory.MemoryPriority.HIGH,
