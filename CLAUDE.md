...
- **Version**: 14.18 (2026-09-08). Added the **promotion-verification package** (AutoResearch-inspired adoptions): cross-model second judge at the promotion chokepoint only — empty-context second-family audit that records an honest `not-configured` (never a fabricated pass) when `ASTRA_JUDGE2_TOKEN`/`ASTRA_JUDGE2_MODEL` are unset and blocks only on grounded known-entailed-with-citation; pilot-before-scale early-stop for cold datasets (zero gate1 passes in the last 120 verdict rows → 3-trial pilot, kind="note" ledger record with reason); negative-results register — a `blocked-at-promotion` stream built from kind="attack" ledger lines (attacker-killed / not-runnable / second-judge-known) with emission outcome (`emitted`, `emit_reason`, attacker + judge projections) now written into the verdict log; prior-art sweep — cached NOVEL novelty verdicts older than the 7-day live TTL fall through to live re-verification before emission. *(v14.17: Eureka external clock + Phase 2 — daily 09:41 clock, fresh-attacker hard rule, calibration lens, live-source check for cached known-verdicts; v14.16: AstroMLab evaluation package; v14.15: model-falsification discovery arm; v14.14: full audit + astro_physics rebuild.)* **Full version history + status detail → `CLAUDE_ASTRA_SYSTEM_STATUS.md`.**
