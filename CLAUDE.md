...
- **Version**: 14.15 (2026-07-18). Added the **model-falsification discovery arm** (`scientific_discovery/falsification/`) — the strategic shift to raise paradigm-shift yield: test *named, cited, code-evaluated* predictions of physical models against real data and flag **anomalies** (deviations surviving statistical + named-systematic + absolute-effect gates), with replicating literature-unexplained anomalies promoted to Eureka-tier. Fiction-safe by construction (predictions are curated+cited+code; LLM narrates only). Seed registry verifies end-to-end: GR perihelion **confirmed** on Mercury (negative control), Newtonian gravity **falsified** (Δ≈1075σ — the canonical paradigm shift). This supersedes catalog correlation-mining as the primary high-leverage discovery mode. *(v14.14: full `astra_core` audit + physics rebuild — revived `astro_physics` (43→0 failures, `AstroSwarmSystem` live), 11 verified PhD-level physics modules, guard widening, ARC-AGI removed, `swarm/` MORK stigmergy confirmed core.)* Importable modules 476→555; baseline 43→28. **Full version history + status detail → `CLAUDE_ASTRA_SYSTEM_STATUS.md`.**
- **GitHub**: https://github.com/Tilanthi/ASTRA-dev


### Current Status
All `System Status — v14.x` sections (v14.0→v14.15: fiction-free two-gate EVALUATE → data lake → niche rotation → dedup → non-optical datasets → self-improvement → residual-seed fix → cross-modal `sdss_wise_xmatch` → anti-circularity gate → repo-health audit → codebase audit + astro_physics physics rebuild → **model-falsification discovery arm**) now live in **`CLAUDE_ASTRA_SYSTEM_STATUS.md`** to keep this entry file lean.
