...
- **Version**: 14.13 (2026-07-17) — repo-health audit (external-review parity): `RASTI_paper/RASTI.cls`→`rasti.cls` (Linux build fix — the paper's `\documentclass{rasti}` is lowercase); 43 pre-existing truncated source files honestly baselined in `astra_core/tests/known_broken_syntax.txt` (broken since the earliest git history — no intact copy to restore, so NOT guess-repaired); `test_syntax_baseline.py` CI guard now blocks any NEW breakage. `import astra_core` is healthy (the sibling build's 33 import-time NameErrors do not occur here). On top of v14.12 (anti-circularity sub-gate), v14.11 (widened `sdss_wise_xmatch` 116→~1,240 rows), v14.10 (first cross-matched dataset: SDSS optical × AllWISE mid-IR per object, opening the optical–IR axis, justified by the v14.9 residual-seeding fix), v14.9 (residual-seeding fix: gate1-pass 25%→34% + data-driven seeding + explored-themes + discovery_review + confirmation-rate CI), v14.8 (Brier calibration + Discovery CI), v14.7 (self-improvement layer), v14.6 (non-optical datasets), v14.5 (near-dup reviewer), v14.4 (effect+p-value dedup), v14.3 (always-on niche rotation), v14.2 (rotation miner + niche hints), v14.1 (data lake + cleanup), v14.0 (fiction-free two-gate EVALUATE).
