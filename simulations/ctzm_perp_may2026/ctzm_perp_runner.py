#!/usr/bin/env python3
"""
CTZM_PERP — Critical Transition Zone Mapping (Perpendicular Field) — May 2026
96 sims: f=[1.2,1.3,1.4,1.5] × beta=[0.3,0.5,1.0,2.0] × M=[1.0,2.0] × seeds=[0,1,2]
Domain: 256×64×64, L=8λ_J, PERPENDICULAR B (θ=90°), HDF5 at dt=0.02 tJ
Binary: /home/fetch-agi/athena/bin/athena  (filament_spacing_pr pgen, existing)

Science question: Does λ/W evolve smoothly across f=1.2-1.5 for PERPENDICULAR fields?
This tests whether the smooth evolution seen in longitudinal CTZM is geometry-independent.
"""
        ...
        for beta in BETA_LIST:
            for mach in MACH_LIST:
                for seed in SEEDS:
                    ...
                    run_id  = f"CTZM_PERP_f{f_str}_b{b_str}_m{m_str}_s{seed}"
                    sims.append({
                        "run_id": run_id,
...
def make_athinput(run_dir: Path, p: dict) -> Path:
    athinput = f"""<comment>
problem   = CTZM_PERP transition zone mapping (perpendicular field)
run_id    = {p['run_id']}
...
theta_deg       = 90.0
"""
    inp = run_dir / "athinput.ctzm_perp"
...
def analyse_and_purge(run_dir: Path, t_frag):
    """Load every HDF5 snapshot, run peak detection, classify, then delete all HDF5."""
...
def main():
    ...
    sims = build_sim_list()
    print(f"CTZM_PERP Campaign: {len(sims)} sims | NP={NP} | MAX_CONC={MAX_CONC}")
    print(f"f: {F_LIST} | beta: {BETA_LIST} | mach: {MACH_LIST} | seeds: {SEEDS}")
    print(f"B-field: PERPENDICULAR (θ=90°)")
    ...
    with ThreadPoolExecutor(max_workers=MAX_CONC) as pool:
        ...
        for fut in as_completed(futures):
            r = fut.result()
            results.append(r)
            (BASE_DIR / "ctzm_perp_results.json").write_text(json.dumps(results, indent=2))
    ...
    print("\n" + "=" * 80)
    print(f"CTZM_PERP CAMPAIGN COMPLETE: {len(results)} sims in {total_wall/60:.1f} min")
    ...
    (BASE_DIR / "ctzm_perp_results.json").write_text(json.dumps(results, indent=2))
    print(f"\nResults: {BASE_DIR}/ctzm_perp_results.json")
