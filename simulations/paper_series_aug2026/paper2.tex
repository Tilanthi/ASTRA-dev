...
A ladder of impulsively contracted, rebounding cylinders supports that reading without establishing a law. Within a fixed transverse domain its resolved modes follow a gravitational $\rho^{-1/2}$ scaling over a decade in density; but enlarging the domain and extending the seeded modes breaks the rank ordering and does not recover it. It is a fixed-box result, not a measured exponent.

The wider outcome is negative. These calculations produce a quasi-periodic bead chain and do not reproduce the observed clustering, and continuously driven turbulence does not repair it: driving raises the fragment count but \emph{lowers} the gap dispersion and the Fano factor, moving the chain further from the observations. What the data require is an inhomogeneous assembly history, not more turbulence.
...
\midrule
\multicolumn{3}{@{}l}{\emph{Load-bearing (108 runs)}}\\
...
Continuous-driving campaign, $L_x=48\,\lambda_{\rm J}$ & 15 & the point-process test of Section~\ref{sec:driven}, Table~\ref{tab:driven} (7 at onset) \\
\midrule
