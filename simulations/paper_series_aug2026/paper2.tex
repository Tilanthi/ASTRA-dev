...
The result rests on a control: a drag-relaxed Ostriker cylinder and a near-critical contracting filament pass through one identical growth-rate measurement. For the equilibrium the pipeline recovers the classical maximum at $22H$ to within $10$ per cent, a resolved peak. For the contracting filament it resolves none: the growth rate rises monotonically to the resolution limit, leaving only the upper limit $\lambda<1.1\,\lambda_{\rm J}$. The distinction between the static and contracting ladder states also survives in a larger transverse domain. For the idealised configuration tested here, classical mode selection is therefore not obligatory.

A ladder of impulsively contracted, rebounding cylinders supports that reading without establishing a law. Within a fixed transverse domain its resolved modes follow a gravitational $\rho^{-1/2}$ scaling over a decade in density, reproducibly between perturbation realisations; but enlarging the domain and extending the seeded modes compresses the density range, breaks the rank ordering and does not recover the scaling. It is a fixed-box result, not a measured exponent.

The magnetic channel is largely untested: a longitudinal field leaves the axial scale unchanged, the ideal perpendicular case collapses radially, and the force-balanced helical equilibrium is not simulated. These calculations produce a periodic bead chain, and do not explain the clustered core distribution of Paper~I.
...
\midrule
\multicolumn{3}{@{}l}{\emph{Load-bearing (93 runs)}}\\
