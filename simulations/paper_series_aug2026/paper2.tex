...
A ladder of impulsively contracted, rebounding cylinders supports that reading without establishing a law. Within a fixed transverse domain its resolved modes follow a gravitational $\rho^{-1/2}$ scaling over a decade in density, reproducibly between realisations; but enlarging the domain and extending the seeded modes breaks the rank ordering and does not recover the scaling. It is a fixed-box result, not a measured exponent.

The wider outcome is negative. Seeded from a statistically homogeneous spectrum on an axially uniform filament, these calculations produce a quasi-periodic bead chain and do not reproduce the observed clustering. The one field geometry able to shorten the mode is not simulated, and driven turbulence is not tested, so what sets the observed distribution remains open.
