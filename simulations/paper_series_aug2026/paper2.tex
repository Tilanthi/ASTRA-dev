...
\midrule
\multicolumn{3}{@{}l}{\emph{Load-bearing (123 runs, plus the analytic helical sequence)}}\\
...
Axially modulated line mass, isothermal & 8 & the inhomogeneous-assembly test of Section~\ref{sec:inhom} \\
Axially modulated line mass, adiabatic & 7 & the same test with a stiffened EOS \\
\midrule
...
\begin{table}
\caption{Load-bearing \emph{production} simulations underlying the fragmentation-wavelength result: the line-mass ladder (campaign~15) and the direct supercritical ensemble (campaign~11). The line-mass ladder uses $\beta=1.0$, $\theta=0^\circ$, a broadband $\mathcal{M}=10^{-3}$ seed and ambient (periodic or reflecting) boundaries, and beads at $t_{\rm frag}\approx1.1$--$1.2\,t_{\rm J}$; the supercritical ensemble spans $f=1.5$--$2.0$, $\beta=0.5$--$2.0$ at $\theta=0^\circ$ with seeds 42 and 137, periodic and reflecting ambient boundaries, and beads at $t_{\rm bead}\approx0.6$--$0.75\,t_{\rm J}$.  These production runs, not the wider exploratory grid, carry the quoted $\lambda_{\rm turn}^{\rm lim}$.}
...
\midrule
\multicolumn{5}{@{}l}{\emph{Line-mass ladder} (campaign~15)}\\
...
\midrule
\multicolumn{5}{@{}l}{\emph{Direct supercritical ensemble} (campaign~11, 39 runs)}\\
...
$W_{\Sigma}$ & FWHM of the \emph{projected column} profile & $4.336\,H$ \\
...
\frac{{\rm d}\rho}{{\rm d}r}\left[c_s^2+\frac{\rho\,(a^2+b^2r^2)}{4\pi}\right]
 &= -\rho\,\frac{{\rm d}\Phi}{{\rm d}r}-\frac{b^2r\rho^2}{2\pi},\\[2pt]
...
The quantitative comparison is not a success, and the reason is specific. Repeating the experiment with an adiabatic equation of state at $f_0=2.5$, so that the cores heat as they compress, and measuring at the same fragmentation-onset threshold, the modulated runs give \emph{fewer} cores than the uniform controls, $4$--$6$ against $8$--$9$, and a lower occupancy, $0.020$--$0.023$ against $0.043$--$0.055$. That is a selection effect of the comparison and not a property of the mechanism: onset is set by whichever patch is densest, so a modulated filament crosses the threshold as soon as its most supercritical stretch collapses, at $t\simeq0.4\,t_{\rm J}$ against $t\simeq1.3\,t_{\rm J}$ for the uniform case, long before the remaining groups have responded.
...
The position is therefore this. An axially varying line mass is the only mechanism we have tested that produces core-free stretches at all, and it produces them for the right reason. Whether it also reproduces the observed Fano factors of $1.6$--$25$ is not settled here, and we can be precise about why. Following every group to fragmentation in one calculation is prevented by a limitation of the method, not of the model: a grid code integrating gravitational collapse without sink particles loses its timestep once the first core forms. We measured the effect rather than assuming it. In the isothermal runs the timestep falls to ${\sim}10^{-20}\,t_{\rm J}$ once any core collapses, and it does so in the uniform controls as well as the modulated ones, so it is not an artefact of the modulation. Stiffening the gas delays the loss but does not prevent it: with an adiabatic equation of state the integration reaches $t\simeq1.35\,t_{\rm J}$ and then stalls, the measured rate of advance implying $\gtrsim10^{2}$ days of wall time to reach $2.5\,t_{\rm J}$, with several runs frozen outright. Sink particles, or a calculation halted and restarted group by group, are required; neither is attempted here.
...
\begin{equation}
N(x)\propto\int_{-\infty}^{\infty}\!\frac{{\rm d}y}{\left[1+\frac{x^2+y^2}{R_{\rm flat}^2}\right]^{2}}
=\frac{\pi}{2}\,\frac{R_{\rm flat}}{\left[1+\left(\frac{x}{R_{\rm flat}}\right)^{2}\right]^{3/2}},
