...
Every homogeneous channel we can reach fails. The calculations give a quasi-periodic bead chain, not the observed clustering; driven turbulence raises the fragment count while \emph{lowering} the gap dispersion and Fano factor; and a toroidally dominated helical equilibrium shrinks the width faster than the wavelength, so $\lambda/W$ rises. Only an axially varying line mass produces core-free stretches, and for the right reason: fragmentation is confined to the locally supercritical parts.
...
The position is therefore this. An axially varying line mass is the only mechanism we have tested that produces core-free stretches at all, and it produces them for the right reason. Whether it also produces the observed occupancy fraction of $6$--$27$ per cent and Fano factors of $1.6$--$25$ is not settled by these calculations, and settling it is a well-posed problem: a refined, longer-duration calculation of Eq.~\ref{eq:fmod} with $\mathcal{A}$ and $\ell$ scanned, compared against Paper~I's statistics. That is a sharper statement of what is missing than the one we could make before, which was only that homogeneous models fail.
