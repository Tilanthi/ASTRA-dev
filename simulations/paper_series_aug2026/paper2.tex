...
An isothermal cylinder in hydrostatic equilibrium fragments fastest at $\lambda_{\rm m}=22H$, the reference against which observed core separations are read. Real filaments contract and accrete as their perturbations grow. We test with ideal-MHD Athena++ calculations whether an evolving filament must select its initial equilibrium wavelength.

The result rests on a control: a drag-relaxed Ostriker cylinder and a near-critical contracting filament pass through one identical growth-rate measurement. For the equilibrium the pipeline recovers the classical maximum at $22H$ to within $10$ per cent, a resolved peak. For the contracting filament it resolves none: the growth rate rises monotonically to the resolution limit, leaving only the upper limit $\lambda<1.1\,\lambda_{\rm J}$. The two states separate cleanly, and still do in a larger transverse domain. For the idealised configuration tested here, classical mode selection is therefore not obligatory.

A ladder of impulsively contracted, rebounding cylinders supports that reading without establishing a law. Within a fixed transverse domain the resolved modes follow a gravitational $\rho^{-1/2}$ scaling over a decade in density, reproducibly between perturbation realisations; but enlarging the domain and extending the seeded modes compresses the density range, breaks the rank ordering and does not recover the scaling. It is a fixed-box result, not a measured exponent.

The magnetic channel is largely untested: a longitudinal field leaves the axial scale unchanged, the ideal perpendicular case collapses radially and yields no axial wavelength, and the force-balanced helical equilibrium is not simulated. These calculations produce a periodic bead chain, and do not explain the clustered core distribution of Paper~I.
...
\item \textbf{The magnetic channel is largely untested.} A uniform longitudinal field leaves the axial scale unchanged over $\beta=0.3$--$2$. The ideal perpendicular-field calculation resolves monolithic radial collapse and shows that the apparent short axial beading at low resolution is numerical; it therefore yields no measured axial fragmentation wavelength. The ambipolar-diffusion perpendicular case remains unresolved. A seeded helical field is an out-of-equilibrium pinch and not a magnetostatic equilibrium. The force-balanced Fiege--Pudritz configuration, the only geometry identified here capable of shortening the axial mode, is not simulated and remains entirely unconstrained by this work.
