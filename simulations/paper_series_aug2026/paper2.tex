...
The outcome is negative on every channel we can reach. These calculations produce a quasi-periodic bead chain, not the observed clustering; driven turbulence raises the fragment count while \emph{lowering} the gap dispersion and the Fano factor; and a toroidally dominated helical equilibrium, the one geometry able to shorten the mode, shrinks the width faster than the wavelength, so the observable $\lambda/W$ rises. The data require an inhomogeneous assembly history.
...
\midrule
\multicolumn{3}{@{}l}{\emph{Load-bearing (108 runs, plus the analytic helical sequence)}}\\
