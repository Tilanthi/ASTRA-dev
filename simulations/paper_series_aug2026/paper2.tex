...
An isothermal cylinder in hydrostatic equilibrium fragments fastest at $\lambda_{\rm m}=22H$, the reference against which observed core separations are read. Real filaments contract and accrete as their perturbations grow. We test, with ideal-MHD Athena++ calculations, whether an evolving filament must select the wavelength of its initial equilibrium.

The central experiment is a control: a drag-relaxed Ostriker cylinder and a near-critical contracting filament pass through one identical growth-rate measurement. For the equilibrium the pipeline recovers the classical maximum at $22H$ to within $10$ per cent. For the contracting filament it resolves no maximum: the growth rate rises monotonically to the resolution limit, so only an upper limit $\lambda<1.1\,\lambda_{\rm J}$ is available.

In a ladder of impulsively contracted, rebounding cylinders spanning a decade in the density at which the modes grow, the selected scale follows that density as a gravitational length: a censored fit over $29$ calculations, with four extra perturbation realisations at each of three densities, gives $-0.48\pm0.05$, consistent with $-1/2$. The mode is reproducible between realisations but shifts by one mode number when the fitting window moves, so this is a scaling and not a precision measurement.

The magnetic channel is largely untested: a longitudinal field leaves the axial scale unchanged over $\beta=0.3$--$2$, the perpendicular case is unresolved, and the force-balanced helical equilibrium, the one geometry able to shorten the mode, is not simulated. We conclude that classical mode selection is not obligatory; but these calculations produce a periodic bead chain, and do not explain the clustered core distribution of Paper~I.
...
\smallskip\noindent\textbf{Structure.} Section~\ref{sec:method} gives the numerical set-up and the initial conditions in full. Section~\ref{sec:control} is the load-bearing experiment: the same measurement applied to a relaxed cylinder and to an evolving one. Section~\ref{sec:ladder} generalises it to a seventeen-run ladder in dynamical state (which we call the \emph{impulsive-contraction ladder}, to distinguish it from the line-mass ladder of Table~\ref{tab:production}) and extracts the scaling. Section~\ref{sec:convergence} sets out the resolution and convergence tests, including a $1024\times256^2$ check. Section~\ref{sec:fields} states what the campaign does and does not constrain about magnetic geometry, and Section~\ref{sec:ambient} what it establishes about ambient confinement. Section~\ref{sec:comparison} makes the comparison with the observations of Paper~I explicit about what it can and cannot mean.
...
\midrule
\multicolumn{3}{@{}l}{\emph{Load-bearing (87 runs)}}\\
...
\begin{table}
\caption{Terms on the simulated wavelength $\lambda_{\rm turn}^{\rm lim}$ and on its conversion to the observable normalisation. The first block is numerical and internal to the calculations; the second enters only when a simulated wavelength is expressed in the units an observer measures, and it dominates. The corresponding terms on the observed spacing itself are given in Paper~I \citep{PaperI}.}
\label{tab:simbudget}
\footnotesize
\setlength{\tabcolsep}{3pt}
...
\smallskip\noindent\textbf{Which hypothesis does this support?} Two must be distinguished. The first is that contraction shortens the selected wavelength directly, as a dynamical effect over and above the change in background state. The second is that the selected wavelength simply follows the instantaneous gravitational scale as the central density evolves, contraction being the means by which that density changes, not an independent wavelength-setting parameter. These experiments support the second much more directly than the first. An exponent of $-1/2$ is the dimensional expectation for a Jeans or scale-height problem, so recovering it demonstrates that mode selection tracks $\rho_{\rm eff}$, not that a new mechanism is operating.
...
\item \textbf{The selected mode follows the instantaneous gravitational scale.} Over $29$ calculations spanning a factor of ten in the density at which the modes grow, and including four independent perturbation realisations at each of three densities, $\lambda_{\rm max}\propto\rho_{\rm eff}^{-0.48\pm0.05}$, consistent with $-1/2$. Evaluated at each run's own density the residual displacement from the equilibrium expectation has a median of $13$ per cent. Evolution changes the physical state at which fragmentation develops; it is not shown to generate an intrinsically shorter wavelength for that state. We present this as a scaling rather than as a precision exponent, because shifting the fitting window moves the selected mode by one mode number, a term comparable to the intrinsic scatter.
...
The calculations are isothermal, use idealised initial profiles, and seed the perturbations once rather than driving them continuously. Each of those omissions could introduce a physical short-wavelength cutoff that these runs cannot see. We identify one specific next calculation rather than a general appeal for more work. An initial-value calculation seeded from a stationary spectrum produces a periodic chain by construction, because every stretch of the filament is seeded identically; that is why these runs cannot reproduce the parsec-scale clustering of Paper~I no matter how the parameters are tuned. Continuous driving breaks that symmetry, because the injected energy acts as a repeated, spatially localised compression which raises the line mass in some stretches and not others while the instability is developing. The required calculation is therefore a continuously driven, non-isothermal MHD run with an injection scale of order a parsec and a line mass that varies along the axis, evolved well past first fragmentation; and the quantity to compare is not a wavelength but the occupancy fraction, the Fano factor and the gap distribution measured in Paper~I. That comparison is only meaningful once the observed network is known to be a physical object, which requires the velocity-resolved dense-gas mapping recommended in Paper~I: N$_2$H$^+$(1--0) or NH$_3$(1,1) at $\lesssim0.05$\,pc resolution. The observational and the numerical programmes therefore converge on the same missing measurement.
