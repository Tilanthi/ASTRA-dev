...
3. **How do we get around it — and the important new evidence.** Two complementary routes, and the first is now producing data:
   - **(a) Converge the perpendicular runs (the definitive fix — IN PROGRESS, no result yet).** ASTRA-PA is running the perpendicular ambipolar configurations at doubled transverse resolution (512×128×128, so N_J ≳ 4). **Status: still running; NOT yet conclusive.** The runs have only reached t ≈ 0.30 t_J and show no beading — **but the 64³ baseline also had no beading at t ≈ 0.30 (it beads at t ≈ 0.45); so this is not yet evidence of suppression, only that the beading epoch has not been reached.** The one weak hint is that the higher-resolution run sits at ~4× higher density contrast at the same time (C ≈ 840 vs 223 at t = 0.30), i.e. radial collapse is proceeding somewhat faster — which *could* pre-empt beading — but this is not established. **A definitive answer requires the runs to reach t ≈ 0.45 (or terminate in radial collapse) for a fair comparison; that result is pending.** Do not rely on it for v56.
...
- **[COMPUTE-RUNNING — not yet conclusive]** The doubled-resolution spot-check is running but has only reached t ≈ 0.30 (beading epoch ≈ 0.45); **no conclusion can be drawn yet** — the 64³ baseline also shows zero beading at t = 0.30. Converged outcome to follow as an addendum; the reframing rests on the Truelove violation alone.
...
- Higher-resolution perpendicular ambipolar rerun (512×128×128, N_J ≳ 4) — M1 / Referee 1 #1 / Referee 2 M1. **Status: running, at t ≈ 0.30, NOT yet conclusive** (beading epoch ≈ 0.45; baseline also shows no beading at t = 0.30, so no suppression can be claimed yet). Converged outcome to follow as an addendum. The v56 reframing does not depend on this and should be applied now on the strength of the existing Truelove violation alone.
...
The two referees are aligned and constructive: **there is no fatal flaw and no large new campaign is needed.** The revision is (1) reframe the perpendicular result as *preliminary/unconverged* (not a demonstrated failure) — which the running higher-resolution runs will eventually settle (that result is not yet in hand); (2) adopt T1 = 0.61 (→ supercritical λ/W = 2.0) and call T1 a provisional calibration; (3) propagate the population-weighted uncertainty and label that synthesis Speculative; (4) put the FilFinder/DisPerSE systematic in the budget; (5) relabel the Figs 9–10 window to the NN statistic; (6) front-load the summary table, cut ~20%, and make the observational sub-Jeans result the clear headline. Items (1), (2) and (4) are the ones both referees will check first.
