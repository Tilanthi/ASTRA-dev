...
\caption{\textbf{Test Case 6: Autonomous Discovery of the Fundamental Metallicity Relation}. ASTRA's discovery-mode causal analysis of the 3,000-galaxy synthetic SDSS sample, operating without prior knowledge of what to find. \emph{Panel (a)}: The Fundamental Metallicity Relation (FMR) discovered autonomously: gas-phase metallicity versus star formation rate, shown separately for five stellar mass bins ($\Delta\log M_*/M_\odot = 0.5$). A negative anti-correlation between metallicity and SFR at fixed mass is detected ($r_{\rm partial} = -0.30$, $p < 10^{-20}$), consistent with the FMR of \citet{mannucci2010}. \emph{Panel (b)}: The FMR signal isolated as the residual $\Delta Z$ from the mean mass-metallicity relation versus SFR, confirming the secondary dependence of metallicity on star formation activity at fixed mass. This secondary correlation was \emph{not} directly encoded in the data-generation procedure, but emerges from the interplay of the main-sequence and mass-metallicity relations. \emph{Panel (c)}: Multi-dimensional outlier detection in stellar mass--specific SFR space; 150 galaxies (5\%) are flagged by the isolation forest, encompassing starbursts, post-starburst candidates, and unusual quenched objects. \emph{Panel (d)}: Causal structure of galaxy properties inferred autonomously by ASTRA, with stellar mass identified as the primary driver of both SFR and metallicity, and the SFR-metallicity FMR link emerging as a secondary relation. This figure demonstrates that ASTRA can detect a genuine, literature-confirmed astrophysical relationship \citep{mannucci2010} without being instructed to search for it.}
\label{fig:test06d}
\end{figure*}
...
\textbf{Compared to Causal ML Approaches}: Recent work has applied causal machine learning directly to astrophysical problems. \citet{mucesh2024} applied doubly-debiased machine learning (DDML) and targeted maximum likelihood estimation (TMLE) to IllustrisTNG hydrodynamical simulations to disentangle the contributions of environment and stellar mass to star formation quenching---a direct application of causal inference methodology to galaxy formation. ASTRA's approach is complementary: while domain-specific causal ML studies target individual scientific questions with bespoke estimators, ASTRA provides a general-purpose integrated framework where causal discovery (via the PC algorithm), Bayesian model selection, dimensional analysis, and multi-wavelength data fusion are available as a coordinated toolkit for a broader range of astrophysical analyses.
...
\item Detected starbursts (74 galaxies above MS), of which 46 were categorized as rapidly transitioning, 2 as post-starburst candidates (low SFR, blue colours), and 28 as quenched below MS --- 74 + 76 = 150 total
