...
The seventh test case demonstrates autonomous HPC simulation campaign management: ASTRA co-ordinated over 1,000 Athena++ MHD simulations on a remote 224-vCPU server across five interlinked campaigns to validate the magnetic Jeans fragmentation criterion for ISM filaments, confirming the predicted stability threshold $\beta_\mathrm{crit} = 2/3$, calibrating the fragmentation wavelength to $(1.11 \pm 0.12)\,\lambda_{J,m}(\theta,\beta)$ across 18 field-geometry configurations, and demonstrating that an adiabatic equation of state completely suppresses near-critical fragmentation, validating the isothermal assumption as the conservative lower bound.
...
computing (HPC) facility. ASTRA co-ordinated over 1,000 Athena++ \citep{stone2020} magnetohydrodynamic
(MHD) simulations across five interlinked campaigns to validate the magnetic Jeans fragmentation
criterion for ISM molecular cloud filaments, characterise the observational bias relevant to
Herschel survey data, and demonstrate robustness across field geometry, near-critical
...
ASTRA co-ordinated five interlinked campaigns on the astra-climate GCE server (224-vCPU AMD
...
To assess robustness and generality, ASTRA co-ordinated an additional 403 Athena++ MHD
simulations: Campaign D (89 sims) addressing near-critical fragmentation (36 sims), field
geometry sensitivity (40 sims), and numerical robustness checks (13 sims); and Campaign E
(314 sims) addressing referee concerns on near-critical field geometry and equation of state
(Section~\ref{sec:tc7_results}). All 403 isothermal simulations fragmented (100\% rate),
while the 30 adiabatic simulations did not, confirming the equation-of-state dependence
discussed above.
...
\textbf{Near-Critical Fragmentation}: 36 simulations at $f \in \{1.00, 1.05\}$,
$\beta \in \{0.3, 0.5, 0.7, 1.0, 1.5, 2.0\}$, and $\mathcal{M} = 1$ confirm that
fragmentation persists even in near-critical filaments (Figure~\ref{fig:tc7_nearcrit}).
Fragmentation timescales extend to $t_\mathrm{frag} \approx 0.88$--$1.63\,t_J$ at
$\beta = 0.3$--$2.0$, a factor of $\sim 5\times$ longer than at $f = 2.0$. The
$t_\mathrm{frag}$ increases by $\sim 1.8\times$ from $\beta = 2.0$ to $\beta = 0.3$,
demonstrating that near-critical filaments are especially sensitive to magnetic support.
These timescales are physically realistic: for $t_J \approx 1.5$--$2.5$~Myr in molecular
clouds, fragmentation occurs within $1.3$--$4.1$~Myr even at $f = 1.00$, consistent with
observed star-formation timescales in thermally near-critical filaments.
...
the system formulated the scientific question, selected simulation parameters, managed over 1,000
HPC jobs on a remote server across five campaigns, analysed output, applied observational bias
corrections, and produced a self-consistent physical interpretation without human intervention at
the computation or analysis stages.
...
By co-ordinating over 1,000 Athena++ simulations across five campaigns on a remote HPC
...
\textbf{Test 7: Autonomous MHD Simulation Campaign} (over 1,000 Athena++ simulations)
\begin{description}
