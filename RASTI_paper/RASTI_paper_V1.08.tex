...
We demonstrate capabilities through six test cases, all using real observational data. The first five validate ASTRA's ability to recover known astrophysical results: Herschel filament scaling relations; Chandra Deep Field South multi-wavelength cross-matching; galaxy property analysis; Gaia stellar causal inference; and Bayesian model comparison.

The sixth test case demonstrates discovery-mode operation on real supernova light curve data (3,000 SNe). Applying ASTRA's V5.0 Discovery Enhancement System, we recover the Phillips relation (decline rate--luminosity correlation), establish its causal structure through temporal analysis, triangulate evidence from numerical correlations, literature, and light curve visualization, and identify high-priority discovery targets including super-Chandrasekhar SNe Ia. This demonstrates discovery-mode capability on real observational data.
...
\item \textbf{Discovery-Mode Operation on Real Data} (Section~8): V5.0 Discovery Enhancement System applied to supernova light curves
\end{enumerate}

\textbf{All six test cases use real observational data}, validating ASTRA's ability to correctly recover known astrophysical results and demonstrating discovery-mode operation on empirical datasets.
...
\textbf{What ASTRA Demonstrates}: The six test cases in this paper show ASTRA can: discover physical laws from data with theoretical validation; fuse multi-wavelength observations with proper uncertainty handling; identify established astrophysical relationships through pattern recognition; distinguish causation from correlation; perform rigorous Bayesian model comparison; and operate in V5.0 Discovery Enhancement System to discover patterns, establish causal structure, and prioritize high-value targets on real observational data.
...
\textbf{Validation Scope}: All six test cases use real observational data to validate ASTRA's ability to correctly recover known astrophysical results and demonstrate discovery-mode operation on real data. The sixth test case demonstrates discovery capability on 3,000 supernovae, with prioritized targets that require future observational validation. The capabilities demonstrated are shown within specific astrophysical domains using particular datasets. Generalization to other domains or datasets requires further validation.
...
\hline
\end{tabular}
\caption{Capabilities demonstrated in each test case. X indicates the capability is explicitly demonstrated in that test case; -- indicates the capability is not applicable or not demonstrated in that test case.}
...
\section{Test Case 6: Discovery-Mode Operation on Real Supernova Data}
...
\textbf{This sixth test case addresses a different question}: can ASTRA demonstrate discovery-mode operation on real observational data? We use a sample of 3,000 supernovae with light curve data to test whether ASTRA's V5.0 Discovery Enhancement System can recover established astrophysical relations, identify causal structure, and generate testable predictions without being told what to look for. This demonstrates discovery-mode capability on real data, not just synthetic benchmarks.
...
To test whether ASTRA can discover causal patterns in real supernova data without being told what to look for, we applied the V5.0 Discovery Enhancement System. This system integrates eight new capabilities with existing ASTRA components:

\noindent\textbf{V101: Temporal Causal Discovery}. Discovers time-lagged causal relationships in light curve evolution, testing whether early-time parameters causally influence peak luminosity.

\noindent\textbf{V102: Scalable Counterfactual Engine}. Tests causal interventions (e.g., ``what if decline rate were faster while holding nickel mass fixed?'') using double machine learning and causal forests.

\noindent\textbf{V103: Multi-Modal Evidence Fusion}. Triangulates evidence from numerical correlations, literature knowledge, light curve visualization, and reproducible analysis code.

\noindent\textbf{V104: Adversarial Discovery System}. Challenges initial discoveries through systematic adversarial testing (confounding, selection effects, measurement error).

\noindent\textbf{V105: Meta-Discovery Transfer}. Applies discovery patterns from other domains (e.g., stellar scaling relations) to supernova light curves.

\noindent\textbf{V106: Explainable Causal Reasoner}. Generates natural language explanations of causal discoveries for astronomical interpretation.

\noindent\textbf{V107: Discovery Triage System}. Prioritizes discoveries by novelty, impact, and confidence to identify high-value targets for follow-up.

\noindent\textbf{V108: Streaming Discovery Engine}. Processes supernova data in streaming mode for real-time classification and anomaly detection.
...
We applied ASTRA's V5.0 system to a sample of 3,000 spectroscopically confirmed supernovae from the Open Supernova Catalog \cite{guillochon2017}, including:
\begin{itemize}
...
ASTRA processed this query through the V5.0 discovery pipeline without being explicitly told about the Phillips relation, nickel mass-luminosity connection, or known SN Ia sub-populations.

\subsection{Results}

\subsubsection{V101: Temporal Causal Discovery}

V101 discovered time-lagged causal relationships in light curve evolution. The key finding: early-time light curve parameters (rise time, early decline rate) causally influence peak luminosity with a time lag of 5--10 days. This supports the physical interpretation that nickel burning and diffusion timescale govern peak luminosity, not just total nickel mass.
...
\subsubsection{V102: Counterfactual Analysis}

V102 tested causal interventions to distinguish correlation from causation:

\begin{itemize}
\item \textbf{Intervention 1}: ``What if decline rate were 0.1~mag faster while holding nickel mass fixed?'' Result: Peak luminosity decreases by $0.12 \pm 0.03$~mag. Average Treatment Effect (ATE) = $-9.21 \pm 1.2$~mag/dex.

\item \textbf{Intervention 2}: ``What if nickel mass were 0.1~dex higher while holding decline rate fixed?'' Result: Peak luminosity increases by $0.08 \pm 0.02$~mag. ATE = $+8.4 \pm 1.5$~mag/dex.

\item \textbf{Intervention 3}: ``What if host galaxy mass were 0.5~dex higher?'' Result: Peak luminosity decreases by $0.05 \pm 0.04$~mag (weak effect). Sensitivity analysis (E-value = 1.3) indicates this may be confounded by metallicity.
\end{itemize}
...
\subsubsection{V103: Multi-Modal Evidence Fusion}

V103 triangulated evidence from four sources:

\noindent\textbf{Numerical evidence}:
\begin{itemize}
\item Decline rate--Peak luminosity: $r = -0.87$, $p < 10^{-50}$ (strong negative correlation)
\item Stretch--Peak luminosity: $r = 0.82$, $p < 10^{-45}$ (strong positive correlation)
\item Host mass--Peak luminosity: $r = -0.31$, $p = 10^{-12}$ (moderate negative correlation)
\end{itemize}

\noindent\textbf{Literature evidence}: V103 retrieved 42 relevant papers confirming the Phillips relation \cite{phillips1993}, stretch-luminosity correlation \cite{hamuy1996}, and host mass dependence \cite{kelly2010}. All three results are well-established in the literature.
...
\subsubsection{V107: Discovery Triage and Prioritization}

V107 ranked discoveries by impact, novelty, and confidence:
...
V107 recommended focusing follow-up resources on super-Chandrasekhar candidates for detailed study (near-infrared spectroscopy, host galaxy analysis, polarimetry).

\subsection{Physical Interpretation}

Figure~\ref{fig:test06a} shows the Phillips relation discovered by ASTRA: decline rate (x-axis) vs peak luminosity (y-axis) for 2,094 SNe Ia. The strong negative correlation ($r = -0.87$) is clearly visible, with faster-declining SNe being fainter.
...
\item \textbf{Causal structure}: Temporal analysis (V101) and counterfactual testing (V102) establish that decline rate causally influences peak luminosity, not just correlates. The physical mechanism is the diffusion timescale: slower-declining SNe have larger diffusion timescales, allowing more efficient photon escape and higher peak luminosity.
...
\item \textbf{Super-Chandrasekhar candidates}: V107 identified 23 SNe Ia ($1.1$\% of the sample) with peak luminosities $>$ 1~mag above the Phillips relation. These are strong candidates for super-Chandrasekhar progenitors ($M > 1.4$~$M_{\odot}$) or different explosion mechanisms (e.g., mergers, core-degenerate scenarios).
...
\textbf{Not a definitive discovery}: This test demonstrates that ASTRA \textit{can} work in discovery mode on real data---recovering known relations, identifying causal structure, and prioritizing high-value targets. It does not claim to have discovered new physics. Definitive scientific discovery requires collaboration with domain experts and follow-up observations.

\begin{figure*}
\centering
\includegraphics[width=0.95\textwidth]{figures/test06_phillips_relation.png}
\caption{\textbf{Test Case 6: Discovery-Mode Operation on Real Supernova Data}. This figure shows ASTRA's V5.0 Discovery Enhancement System applied to 3,000 supernovae. \textbf{Panel A}: The Phillips relation---decline rate vs peak luminosity for 2,094 SNe Ia---showing the strong negative correlation ($r = -0.87$) discovered by ASTRA without prior knowledge. \textbf{Panel B}: Host galaxy stellar mass vs peak luminosity, showing the moderate negative correlation ($r = -0.31$) indicating environmental effects. \textbf{Panel C}: Stretch distribution for SNe Ia, revealing the diversity of light curve shapes. \textbf{Panel D}: V107 triage results prioritizing super-Chandrasekhar candidates (high novelty + impact) for follow-up observations. All patterns were discovered on real observational data, demonstrating discovery-mode capability beyond synthetic benchmarks.}
\label{fig:test06a}
\end{figure*}
...
\textbf{Discovery Test (Case 6)}: Test Case 6 demonstrates a different capability---discovery-mode operation on real observational data. By applying ASTRA's V5.0 Discovery Enhancement System to a sample of 3,000 supernovae, ASTRA recovered the Phillips relation (decline rate--luminosity correlation), established its causal structure through temporal analysis and counterfactual testing, triangulated evidence from numerical correlations, literature, and light curve visualization, and identified high-priority discovery targets including super-Chandrasekhar SNe Ia. This test addresses a key question: can ASTRA go beyond ``correct recovery of known results'' to demonstrate discovery-mode capability on real data? The results suggest that ASTRA's architecture supports discovery-mode operation on real observational data, though definitive scientific discovery requires collaboration with domain experts and follow-up observations.
...
\item \textbf{Discovery mode} (Test 6): ASTRA analyzes real observational data (3,000 supernovae) using V5.0 Discovery Enhancement System to discover patterns, establish causal structure, and prioritize high-value targets without being told what to look for.
\end{enumerate}

These test cases should be understood as demonstrations of technical capabilities. For Tests 1--5, the value lies in showing ASTRA can correctly recover known results using an integrated framework. For Test 6, the value lies in showing ASTRA can work in discovery mode on real data, though the prioritized targets require follow-up observations.
...
\textbf{Test 6: Discovery-Mode Operation} (3,000 supernovae)
\begin{itemize}
\item Recovered Phillips relation (decline rate--luminosity correlation) with $r = -0.87$, $p < 10^{-50}$
...
\item Demonstrated discovery-mode capability on real observational data
\end{itemize}

\textbf{All Six Test Cases Use Real Data}: All six test cases use real observational data, demonstrating discovery-mode capabilities on real observational data (3,000 supernovae).

\textbf{Two Modes of Operation}: Tests 1--5 validate ASTRA's ability to correctly recover known astrophysical results using real data. Test 6 demonstrates ASTRA's discovery-mode operation on real observational data using V5.0 Discovery Enhancement System, including temporal causal discovery, counterfactual analysis, multi-modal evidence fusion, and discovery triage.
...
Large language model assistance (Anthropic Claude) was used in manuscript preparation of this complex paper, including drafting initial ideas, editing, and multiple rounds of peer review simulation. This paper was not written by AI, but its evolution was helped by the use of AI tools, following the spirit that ASTRA is a tool able to be used by an Astronomer. All scientific content, data analysis, and conclusions were verified and are the responsibility of the authors following extensive internal review. The Authors declare no conflict of interest.
