#!/usr/bin/env python3
"""
Final Robust Nearest-Neighbor Analysis for HGBS Regions

Handles all catalog formats correctly.
"""
...
def load_catalog_split(catalog_file: str) -> List[Dict]:
    ...
    for line in lines[data_start:]:
        ...
        try:
            ...
            # Dec: +DD MM SS or -DD MM SS (sign with degrees)
            dec_d = parts[5]  # '+DD' or '-DD'
            dec_m = parts[6]  # 'MM'
            dec_s = parts[7]  # 'SS'
...
def load_catalog_csv(catalog_file: str) -> List[Dict]:
    ...
    with open(catalog_file, 'r') as f:
        ...
        for row in reader:
            ...
            try:
                # IC5146 format: id, mass, peak, npix, ra_deg, dec_deg, Tdust, type
                if len(row) >= 6:
                    obj_id = int(row[0])
                    ra_deg = float(row[4])  # ra_deg is column 4
                    dec_deg = float(row[5])  # dec_deg is column 5
...
def main():
    ...
    print("="*80)
    print("FINAL ROBUST HGBS NEAREST-NEIGHBOR ANALYSIS")
    print("="*80)
    ...
    # Save results
    if all_results:
        output_file = base_path / 'nn_analysis_results_final.json'
        ...
        if comparisons:
            ...
            if n_nn_larger == len(comparisons):
                ...
                print("The NN values are SMALLER than pairwise values.")
                print()
                print("INTERPRETATION:")
                print("1. The pairwise median may be measuring a different quantity than NN.")
                print("   - Pairwise: median of all inter-core distances (including cross-filament)")
                print("   - NN: median of nearest-neighbor distances (within-filament)")
                print()
                print("2. For regions with complex filament networks (like Orion B with N=1844),")
                print("   the pairwise median includes cross-filament distances which are larger,")
                print("   while NN measures only the closest adjacent cores along each filament.")
                print()
                print("3. The paper's pairwise values may represent filament-ordered cores")
                print("   rather than all-pairs, which would explain the discrepancy.")
