...
def main():
    ...
    try:
        catalog_data = load_catalog_with_encoding(CAT_FILES['derived'], encoding='latin-1')
    ...
    if catalog_stats and catalog_stats['n_cores'] > 0:
        print(f"  - Total cores: {catalog_stats['n_cores']}")
        ...
        if len(catalog_stats['masses']) > 0:
            print(f"  - Mass range: {np.min(catalog_stats['masses']):.2f} - {np.max(catalog_stats['masses']):.2f} Msun")
