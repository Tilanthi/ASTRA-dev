#!/usr/bin/env python3
"""
Create Polaris Core Catalog

Creates a functional Polaris catalog by using source detection
on the column density map. Polaris has an existing skeleton map.
...
OUTPUT_CAT = os.path.join(HGBS_DIR, 'core_catalog_polaris_derived.csv')

DISTANCE_PC = 150.0  # Distance to Polaris
MIN_NH2 = 1e21  # Minimum column density to consider

print("="*70)
print("Creating Polaris Core Catalog")
print("="*70)
...
for i in range(1, num_features + 1):
    ...
    try:
        ...
        # Scale coordinates to temperature map resolution if needed
        ...
        if 0 <= tx < temp_data.shape[1] and 0 <= ty < temp_data.shape[0]:
            temp = float(temp_data[ty, tx])
        else:
            temp = 15.0  # Default temperature

        # Classify based on density and mass
        # Polaris is a high-latitude cirrus region, generally quiescent
        if mean_nh2 < 5e21:
            core_type = 'starless'
            if temp < 15:
                temp = 16.0
...
with open(OUTPUT_CAT, 'w') as f:
    f.write("# Polaris Derived Core Catalog\n")
...
# Statistics
print("\n" + "="*70)
print("POLARIS DERIVED CATALOG STATISTICS")
print("="*70)
