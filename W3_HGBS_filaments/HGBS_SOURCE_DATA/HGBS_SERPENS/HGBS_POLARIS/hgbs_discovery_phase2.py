#!/usr/bin/env python3
"""
HGBS Polaris Discovery Science - Phase 2: Core-Filament Association
...
# DATA PATHS
# ============================================================================
HGBS_DIR = '/Users/gjw255/astrodata/SWARM/ASTRA/HGBS_SERPENS/HGBS_POLARIS'

# FITS files
COL_DEN_FILE = os.path.join(HGBS_DIR, 'HGBS_polaris_column_density_map.fits')
TEMP_FILE = os.path.join(HGBS_DIR, 'HGBS_polaris_dust_temperature_map.fits')
SKELETON_FILE = os.path.join(HGBS_DIR, 'HGBS_polaris_skeleton_map.fits')

# Catalog file
CAT_FILE = os.path.join(HGBS_DIR, 'core_catalog_polaris_derived.csv')
...
class CoreFilamentAnalyzer:
    ...
    def __init__(self):
        ...
        print("Initializing HGBS Core-Filament Analyzer for Polaris...")
    ...
    def parse_catalog(self):
        ...
        from parse_polaris_catalog import parse_polaris_catalog
        self.cores = parse_polaris_catalog(CAT_FILE)
        print(f"  Loaded {len(self.cores)} cores")

        # Verify core types were extracted
        types = {}
        for core in self.cores:
            ctype = core.get('type', 'unknown')
            types[ctype] = types.get(ctype, 0) + 1
        print(f"  Core types: {types}")
    ...
    def convert_coordinates(self):
        ...
        for core in self.cores:
            try:
                # Use decimal degree coordinates directly from parser
                ...
                # Create SkyCoord object from decimal degrees
    ...
    def calculate_filament_distances(self):
        ...
        for core in self.cores_with_coords:
            ...
            try:
                ...
                # At 150 pc distance
                dist_pc = 150 * np.tan(np.deg2rad(pix_size_deg * 3600)) * dist
    ...
    def extract_local_properties(self):
        ...
        # Calculate scale factors for temperature map (different resolution)
        temp_scale_x = self.temp_data.shape[1] / self.col_den_data.shape[1]
        temp_scale_y = self.temp_data.shape[0] / self.col_den_data.shape[0]

        for core in self.cores_with_coords:
            x, y = int(core['x_pix']), int(core['y_pix'])

            # Ensure coordinates are within bounds for column density map
            ...
            # Extract local temperature (scale coordinates to temperature map resolution)
            tx = int(x * temp_scale_x)
            ty = int(y * temp_scale_y)

            if 0 <= tx < self.temp_data.shape[1] and 0 <= ty < self.temp_data.shape[0]:
                core['local_temp'] = float(self.temp_data[ty, tx])
            else:
                core['local_temp'] = np.nan
        ...
        print(f"  Median local N_H2: {np.median([c['local_nh2_21'] for c in self.cores_with_coords]):.2f}e21 cm^-2")
        valid_temps = [c['local_temp'] for c in self.cores_with_coords if not np.isnan(c.get('local_temp', np.nan))]
        if valid_temps:
            print(f"  Median local T: {np.median(valid_temps):.2f} K")
    ...
    def _pixels_to_pc(self, pixel_dist):
        """Convert pixel distance to parsecs at 150 pc."""
        try:
            ...
            # Distance to cloud (150 pc)
            dist_pc = 150 * pix_size_rad * pixel_dist
            return dist_pc
        except:
            return pixel_dist * 0.0022  # Approximate
    ...
    def run_analysis(self):
        ...
        print("\n" + "="*70)
        print("HGBS Polaris - PHASE 2: CORE-FILAMENT ASSOCIATION")
        print("="*70)
