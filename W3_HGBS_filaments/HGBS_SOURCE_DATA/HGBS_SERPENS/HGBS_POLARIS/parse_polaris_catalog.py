#!/usr/bin/env python3
"""
Parse Polaris Derived Core Catalog (CSV Format)

The Polaris derived catalog is in CSV format with columns:
...
def parse_polaris_catalog(filename):
    """Parse the Polaris derived core catalog from CSV file."""
    cores = []
...
def parse_line(parts, line_num):
    """Parse a single line from the Polaris derived catalog."""
    try:
        ...
        core_name = f'POLARIS_{obj_id:04d}'
...
if __name__ == '__main__':
    catalog_file = '/Users/gjw255/astrodata/SWARM/ASTRA/HGBS_SERPENS/HGBS_POLARIS/core_catalog_polaris_derived.csv'

    print("="*70)
    print("Parsing Polaris Derived Core Catalog")
    print("="*70)

    cores = parse_polaris_catalog(catalog_file)
