#!/usr/bin/env python3
"""
Create Orion A Core Catalog and Skeleton Map

Creates skeleton map and source catalog for Orion A using
...
SKELETON_OUTPUT = os.path.join(HGBS_DIR, 'HGBS_orionA_skeleton_map.fits')
OUTPUT_CAT = os.path.join(HGBS_DIR, 'core_catalog_orionA_derived.csv')

DISTANCE_PC = 420.0  # Distance to Orion A
MIN_NH2 = 1e21  # Minimum column density to consider

print("="*70)
print("Creating Orion A Core Catalog and Skeleton Map")
print("="*70)
...
for i in range(1, num_features + 1):
    ...
    try:
        ...
        # Orion A is an active star-forming region
        if mean_nh2 < 5e21:
            core_type = 'starless'
            if temp < 15:
                temp = 16.0
            ...
            # High density regions in Orion A often have protostars
            # Use temperature to distinguish
            if temp < 20:
                core_type = 'prestellar'
                temp = 11.0
            else:
                # Warm cores likely protostellar
                core_type = 'protostellar'

        cores.append({
            'id': i,
            'name': f'ORIONA_{i:04d}',
            'ra': ra_str,
            'dec': dec_str,
            'ra_deg': ra_deg,
            'dec_deg': dec_deg,
...
with open(OUTPUT_CAT, 'w') as f:
    f.write("# Orion A Derived Core Catalog\n")
...
# Statistics
print("\n" + "="*70)
print("ORION A DERIVED CATALOG STATISTICS")
print("="*70)
