#!/usr/bin/env python3
"""
Parse Orion A Derived Core Catalog (CSV Format)

The Orion A derived catalog is in CSV format with columns:
...
def parse_oriona_catalog(filename):
    """Parse the Orion A derived core catalog from CSV file."""
    cores = []
...
def parse_line(parts, line_num):
    """Parse a single line from the Orion A derived catalog."""
    try:
        ...
        core_name = f'ORIONA_{obj_id:04d}'
...
if __name__ == '__main__':
    catalog_file = '/Users/gjw255/astrodata/SWARM/ASTRA/HGBS_SERPENS/HGBS_ORIONA/core_catalog_orionA_derived.csv'

    print("="*70)
    print("Parsing Orion A Derived Core Catalog")
    print("="*70)

    cores = parse_oriona_catalog(catalog_file)
