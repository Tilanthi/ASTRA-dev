#!/usr/bin/env python3
"""
Parse Pipe Derived Core Catalog (CSV Format)

The Pipe derived catalog is in CSV format with columns:
...
def parse_pipe_catalog(filename):
    """Parse the Pipe derived core catalog from CSV file."""
    cores = []
...
def parse_line(parts, line_num):
    """Parse a single line from the Pipe derived catalog."""
    try:
...
if __name__ == '__main__':
    catalog_file = '/Users/gjw255/astrodata/SWARM/ASTRA/HGBS_SERPENS/HGBS_PIPE/core_catalog_pipe_derived.csv'

    print("="*70)
    print("Parsing Pipe Derived Core Catalog")
    print("="*70)

    cores = parse_pipe_catalog(catalog_file)
