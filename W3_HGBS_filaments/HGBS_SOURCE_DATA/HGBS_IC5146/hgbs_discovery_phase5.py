#!/usr/bin/env python3
"""
HGBS Taurus Discovery Science - Phase 5: Discovery Mode with ASTRA

This script applies ASTRA's discovery capabilities to the HGBS Taurus data:
...
class ASTRADiscoveryEngine:
    ...
    def detect_anomalies(self):
        ...
        # Anomaly type 3: Bonnor-Ebert ratio outliers (skip if not available)
    ...
    def causal_inference_analysis(self):
        ...
        # Collect data for causal analysis - exclude alpha_be if not available
        has_alpha = any(c.get('alpha_be') is not None for c in self.cores)
        var_names = ['mass', 'temp', 'local_nh2']
        if has_alpha:
            var_names.append('alpha_be')
        ...
        if prestellar_nh2 and starless_nh2:
            ...
            print(f"  Insufficient data for prestellar vs starless comparison")
    ...
    def run_discovery_mode(self):
        ...
        print("\n" + "="*70)
        print("HGBS IC5146 - PHASE 5: DISCOVERY MODE WITH ASTRA")
        print("="*70)
        ...
        causal_result = self.causal_inference_analysis()
        if causal_result:
            causal_data, var_names = causal_result
        else:
            causal_data, var_names = None, None
