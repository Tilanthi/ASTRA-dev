\documentclass[twocolumn,longtitle,usenatbib]{mnras}
...
\title{Why Ideal Isothermal MHD Fails to Reproduce Filament Fragmentation: Complete HGBS Analysis and 2,860 MHD Simulations}
...
\begin{abstract}
Classical theory predicts isothermal gas cylinders fragment at a wavelength of $\sim4\times$ their width. We test this prediction using complete HGBS observations (8 regions, 5,069 cores, Gaia DR3 distances) combined with 2,860 self-gravitating MHD simulations---the most extensive numerical investigation of filament fragmentation to date.

\textbf{Primary finding: Ideal isothermal MHD fails under realistic conditions}. The Realistic Turbulence Campaign (RTC) comprises 1,200 simulations with physical ISM turbulence (Mach 2--4) and free boundary conditions. This campaign---the most physically credible in our dataset---produces \textbf{zero} simulations within the HGBS observational window. All measured $\lambda/W$ values exceed the HGBS upper bound by $\geq$22\% (minimum 3.75, mean 10.8, median 7.3). Ideal isothermal MHD cannot reproduce HGBS core spacings under realistic free-boundary conditions.

\textbf{Observational limitations}. Pairwise median statistics give $\lambda/W \approx 2.8$, but for large filaments (Orion B: $N = 1,844$) this statistic converges to $L/3$ and may measure overall filament scale rather than true fragmentation wavelength. Published nearest-neighbour analyses report $\lambda/W \approx 2.0$--$2.2$ using different methodologies. The 3D-corrected pairwise median ($\lambda/W \approx 3.3$--$3.9$) encompasses the classical $4\times$ prediction. Which statistic measures the true fragmentation wavelength requires HGBS skeleton data for proper nearest-neighbour analysis.

\textbf{Why the model fails: Three fundamental tensions}. (1) \textbf{Extrapolation gap}: Quantitative comparisons rely on extrapolating from near-critical simulations ($f \approx 1.0$--$1.2$ where $\lambda/W$ is measurable) to the supercritical regime ($f \approx 1.5$--$3.0$ where HGBS filaments exist). This $\pm$20--30\% extrapolation uncertainty dominates the systematic error budget. (2) \textbf{Perpendicular-field crisis}: Planck (2016) found $\sim$90\% of dense filaments are perpendicular to the mean field. Our perpendicular-field simulations predict $\lambda/W \approx 1.25$ (or $\approx 2.0$--$2.4$ after width normalisation), below both nearest-neighbour observations ($\approx 2.0$--$2.2$) and pairwise median ($\approx 2.8$). The dominant filament population should produce shorter spacings than observed. (3) \textbf{Radial collapse dominates}: All 654 free-boundary supercritical simulations ($f \geq 1.5$) undergo pure radial collapse rather than longitudinal fragmentation, preventing direct $\lambda/W$ measurement in the HGBS regime. Artificial radial confinement (reflecting walls) produces HGBS-compatible spacing ($\lambda/W = 2.65 \pm 0.57$), but real filaments show no observational signatures of such confinement.

\textbf{Implications}. The failure of ideal isothermal MHD under realistic conditions suggests that missing physics---non-ideal MHD effects, time-dependent thermodynamics, or hierarchical fiber fragmentation---is required to explain HGBS core spacings. Resolving whether filaments fragment at the classical $4\times$ scale requires: (1) proper nearest-neighbour analysis with HGBS skeleton data; (2) field geometry measurements at fragmentation scales; (3) bridging the extrapolation gap ($f \approx 1.2$--$1.5$); (4) assessment of non-ideal MHD effects.
\end{abstract}
...
\textbf{Migration effects}. Observational studies suggest typical protostellar migration scales of $0.01$--$0.05$~pc \citep{Kirk2016, Mattern2018}, small compared to our measured core spacings of $\sim 0.3$~pc. Monte Carlo simulations for Orion B (N = 1,844) show that the pairwise median statistic is insensitive to migration: bias $< 0.01\%$ at migration distances up to $0.05$~pc because the statistic is dominated by non-adjacent pairs. However, this reflects the statistic's insensitivity rather than absence of migration effects. For nearest-neighbour statistics (more appropriate for fragmentation wavelength), migration could introduce $\sim$10--20\% bias. We cannot constrain this with pairwise median, adding $\sim$10\% systematic uncertainty to $\lambda/W$ measurements.
...
\textbf{Uncertainty quantification}. Bootstrap resampling (10,000 iterations) and jackknife verification both show that formal statistical errors underestimate the true uncertainty by a factor of $\sim$1.3--1.5 due to region-to-region variation. We adopt bootstrap 95\% confidence interval half-widths as our primary uncertainties: $\lambda/W = 2.79 \pm 0.19$ (full sample) and $\lambda/W = 2.84 \pm 0.12$ (robust regions).

\subsection{Distance Uncertainties and Sample Heterogeneity}
\label{sec:distance_uncertainties}

\textbf{Distance uncertainties and sample classification}. The exceptional +76\% distance revision for Serpens raises concerns about large revisions for regions with small YSO samples. Extinction-based distances independently support the larger Serpens distance (440--460~pc). To address distance uncertainty concerns, we classify regions into two categories (Table~\ref{tab:sample}): \textbf{Robust regions} (Orion B, Aquila, Perseus, Taurus) with large YSO samples ($N > 500$), well-established distances with VLBI validation where available, and either small distance revisions ($<15$\%) or good agreement with prior estimates; and \textbf{Limited regions} (Ophiuchus, Serpens, TMC1, CRA) with smaller YSO samples ($N < 550$) and/or large distance revisions ($>50$\%). We adopt the robust-only result ($\lambda/W = 2.84 \pm 0.12$) as our primary measurement.

\textbf{Robustness to distance errors}. A leave-one-out analysis shows that excluding any single region changes the weighted mean by $<$6\% (Serpens exclusion: 1.1\%). Even the conservative lower bound—reverting to original HGBS distances for limited regions—gives $\lambda/W = 2.68$, 33\% below the classical $4\times$ prediction. VLBI maser parallaxes validate the Gaia DR3 distances for Orion B (386~pc), Perseus, and Ophiuchus to within 1--2\%. The fact that Orion B ($+$48\% revision) and Taurus ($-$4\% revision) undergo revisions in opposite directions, with Orion B independently validated, argues against a coherent systematic bias in the Zhang et al. methodology.

\textbf{Threshold sensitivity}. Testing alternative robust/limited classification thresholds shows the primary result changes by $<$6\% for thresholds ranging from $N > 300$ to $N > 700$, confirming that our conclusions do not depend on the specific $N > 500$ choice.
...
\midrule
Realistic Turbulence Campaign (RTC) & 1,200 & Physical ISM turbulence (Mach 2--4) & \textbf{Zero HGBS matches}, $\lambda/W\geq3.75$ & 4.3 \\
Field Geometry Campaign & 314 & $\theta=0^{\circ}\mbox{--}90^{\circ}$, $f=1.5\mbox{--}3.0$ & Perpendicular-field: $\lambda/W\approx1.25$ crisis & 4.4 \\
Supercritical campaign & 654 & $f=1.5\mbox{--}3.0$, $\beta=0.3\mbox{--}5.0$, $\mathcal{M}=0.5\mbox{--}3$ & Radial collapse dominates (no $\lambda/W$) & 4.5 \\
Definitive Transition Campaign (DTC) & 540 & $f=1.4\mbox{--}2.2$, $\beta=0.3\mbox{--}1.3$, $\mathcal{M}=1\mbox{--}5$ & No stable configurations for $\mathcal{M}\geq1$ & 4.6 \\
Base grid & 208 & $\mathcal{M}=0.5\mbox{--}5$, $\beta=0.05\mbox{--}5$ & Three-regime framework & 4.7 \\
Rigid cylinder campaign & 45 & Reflecting walls, $f\geq2.6$ & $\lambda/W=2.65\pm0.57$ (artificial BCs) & 4.8 \\
Validation campaigns & 83 & Resolution, IC, EOS tests & $128^3\to256^3$: 7.2\% $t_{\rm frag}$ shift & 4.9 \\
Additional campaigns & 432 & Targeted parameter spaces (Apr--Jun 2026) & Turbulence, EOS, width studies & 4.10 \\
\midrule
...
\subsection{PRIMARY RESULT: RTC Null Result}
\label{sec:rtc_primary}

To address whether linear-regime turbulence ($\delta v/c_s = 10^{-4}$) adequately represents real molecular cloud conditions, we conducted the Realistic Turbulence Campaign (RTC)---1,200 simulations with physical ISM turbulence amplitudes ($\delta v/c_s = 1.0$, Mach 2--4) and free boundary conditions, testing both compressive and solenoidal driving modes.

\textbf{Critical finding: Zero HGBS matches}. The RTC produces \textbf{zero} simulations within the HGBS observational window ($[2.52, 3.08]$). All measured $\lambda/W$ values lie above the HGBS upper bound. The minimum measured value is $\lambda/W = 3.75$ (22\% above the HGBS upper bound), with mean $\lambda/W = 10.8 \pm 8.4$ (median 7.3). A broader search window $[2.5, 4.0]$ identifies 4 simulations ($\lambda/W = 3.75$--$3.96$), but none fall within the rigorously defined HGBS bounds. The physically-motivated subspace (longitudinal fields, $f \in [1.2, 2.0]$, $\beta \in [1, 2]$, $M \in [1, 3]$) shows 50 valid $\lambda/W$ measurements with range $[3.96, 19.0]$ (median 7.3)—all above HGBS values.

\textbf{Why this is the primary result}. The RTC is the most physically credible campaign: it uses physical ISM turbulence amplitudes, free boundary conditions, and spans the full relevant parameter space. Under the most realistic conditions we can simulate (ideal isothermal MHD, physical turbulence, free boundaries), the model cannot reproduce HGBS core spacings. This decisive zero-match rate (0/1,200) demonstrates that ideal isothermal MHD is insufficient to describe HGBS filament fragmentation.

\textbf{Fragmentation suppression at physical amplitudes}. Approximately 90\% of filaments undergo radial gravitational collapse rather than axial (beading) fragmentation at physical Mach numbers, regardless of driving mode. Physical ISM turbulence makes HGBS-matching fragmentation rarer than linear-regime turbulence suggests, confirming that our linear-regime results were conservative, not optimistic.
...
% Figure removed (validation detail): DTC re-run with extended timeout
% All 15 STABLE points fragmented with 6-hour timeout, confirming timeout artifacts.
% Original 600-second limit was insufficient for physical fragmentation timescales.
...
\textbf{Turbulence dependence}: Extension campaigns confirm that $t_{\rm frag}$ is completely independent of Mach number within $\mathcal{M} = 0.5$--$3.0$ ($\langle t_{\rm frag} \rangle = 0.2869\,t_{\rm J}$ for all $\mathcal{M}$). This demonstrates that turbulence (at the seeded amplitude of $\delta v = \mathcal{M} \times c_s \times 10^{-4}$) has negligible effect on the collapse timescale. The observed sub-Jeans spacing in HGBS filaments reflects the underlying gravitational instability timescale rather than turbulent fragmentation.

\begin{figure*}
\includegraphics[width=0.7\textwidth]{figures/fig1_tfrag_combined.pdf}
\caption{Fragmentation onset time $t_{\rm frag}$ versus line-mass fraction $f$ for all $\beta$ (colour) and $\mathcal{M}$ (marker). The power-law fit $1/t_{\rm frag} \propto f^{0.39}$ ($r^2 = 0.999$) is shown as the dashed line. The $f$-scaling dominates over $\beta$ and $\mathcal{M}$ effects.}
\label{fig:tfrag_combined}
\end{figure*}

% Figure removed (shows no dependence): Mach number and high-$\beta$ extensions
% Extension campaigns confirm complete independence of Mach number ($\mathcal{M} = 0.5$--$3.0$)

% Figure moved to appendix: Near-critical regime t_frag
% Even marginally supercritical filaments ($f = 1.1$--$1.3$) fragment on timescales $\lesssim 0.4\,t_{\rm J}$.
...
% Figure removed (calibration detail): Oblique field calibration
% Smooth decrease from $0.604\,t_{\rm J}$ ($30^\circ$) to $0.454\,t_{\rm J}$ ($60^\circ$)

\subsubsection{Phase 4: Adiabatic EOS Validation (30 simulations)}

Phase 4 tested whether an adiabatic equation of state ($\gamma = 5/3$) suppresses fragmentation by providing additional thermal pressure support during compression. All 30 simulations ($f = 1.00$--$1.20$, $\beta = 0.5$--$1.0$, $\mathcal{M} = 1.0$--$2.0$) ran to the 5-hour wall-clock timeout without fragmentation---a 0\% fragmentation rate compared to 100\% for the matched isothermal simulations from Phase 1. The isothermal simulations fragment at $t_{\rm frag} \approx 1.08\,t_{\rm J}$ (median), while the adiabatic simulations remain stable at $t_{\rm final} > 300$ minutes (many reaching $t > 30$--$40\,t_{\rm J}$). Representative density profiles show that adiabatic filaments remain smooth with no longitudinal density peaks even after $t = 40\,t_{\rm J}$.

% Figures moved to appendix: Adiabatic EOS validation
% Isothermal: 100\% fragmentation at $t_{\rm frag} \approx 1.08\,t_{\rm J}$
% Adiabatic: 0\% fragmentation, remains smooth at $t > 40\,t_{\rm J}$
...
\label{sec:rtc}
\textbf{Note: The primary RTC result is presented in Section~\ref{sec:rtc_primary}. This subsection contains additional detailed analysis.}
...
\subsection{The Perpendicular-Field Crisis}

An independent and more severe theoretical tension emerges from field geometry considerations. Planck (2016) found that $\sim$90\% of dense filaments are perpendicular to the mean magnetic field. Our perpendicular-field simulations predict $\lambda/W \approx 1.25$ (raw) or $\approx 2.0$--$2.4$ (width-normalised), below both nearest-neighbour observations ($\approx 2.0$--$2.2$) and pairwise median ($\approx 2.8$). This creates an \textbf{independent theoretical crisis}: the dominant filament population should produce spacings substantially shorter than observed.

For longitudinal fields, $\lambda/W$ decreases from $4.74 \pm 0.73$ ($\beta = 0.3$) to $2.80 \pm 0.14$ ($\beta = 2.0$), constraining HGBS filaments with $\lambda/W \approx 2.8$ to $\beta \approx 1.8$--$2.0$ \textit{if} the field is longitudinal. However, the hourglass correction (\citet{Jadhav2026}) gives $\lambda/W \approx 1.6$--$1.8$ for $\theta_{\rm eff} \approx 30^{\circ}$--$45^{\circ}$, which widens rather than closes the gap. No combination of width normalisation and plasma $\beta$ fully reconciles perpendicular-field predictions with HGBS observations. The width-normalised perpendicular-field prediction ($\approx 2.0$--$2.4$) approaches but remains below observed values, and this apparent convergence may be a normalisation artifact rather than physical.
