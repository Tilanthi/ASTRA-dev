...
\textbf{Distance scaling}: For regions spanning distances from 130--400 pc, the same angular resolution thresholds correspond to different physical scales. The Herschel SPIRE 250$\mu$m beam of 18$''$ corresponds to physical scales of 0.011 pc (130 pc) to 0.035 pc (400 pc). To maintain consistency, we applied a distance-dependent scaling to the persistence threshold such that the minimum physical scale probed remains approximately 0.05 pc across all regions. \textbf{Supplementary documentation}: The specific parameter values applied to each region, including the distance-dependent threshold scalings, are documented in the analysis code repository (to be made available upon acceptance).
...
\end{tabular}
\vspace{-0.5em}
\small{$^{\dagger}$For W3 specifically, beam dilution adds an additional systematic uncertainty of approximately 5\% ($\pm$0.010 pc) due to uncertainty in the quadrature correction methodology.}
\end{table}
...
Core spacing measurements for all HGBS regions are summarized in Table~\ref{tab:all_regions}. The appearance of uniformity is driven primarily by the four robust regions (Orion B, Aquila, Perseus, Taurus), which have small standard errors and carry most of the statistical weight. The four limited-sample regions (Serpens, TMC1, CRA, Ophiuchus) have larger uncertainties and are consistent with values anywhere from $\sim0.07$ to $\sim0.35$ pc at $2\sigma$.
...
W3$^{\dagger}$ & 0.222 & 0.038 & 42  & Preliminary \\
\midrule
...
\end{tabular}
\vspace{-0.5em}
\small{$^{\dagger}$W3 is preliminary (see text). TMC1 is a separate subregion within the Taurus cloud complex; the Taurus entry in this table refers to the full Taurus HGBS region as analyzed in the published catalogue.}
\end{table}

\noindent\textbf{Primary result (8 regions)}: The weighted mean core spacing across the 8 high-confidence HGBS regions is $0.211 \pm 0.007$ pc (statistical), corresponding to $2.11\times$ the characteristic filament width. A sensitivity analysis using only the 4 robust regions (Orion B, Aquila, Perseus, Taurus) yields $0.210 \pm 0.008$ pc, demonstrating that the limited-sample regions contribute negligibly to the weighted mean while formally being treated as independent measurements. Including the preliminary W3 measurement (0.222 pc after beam correction) changes the weighted mean to $0.212 \pm 0.007$ pc -- a difference of $<1$\%, demonstrating robustness.

We adopt the 8-region result as our primary finding, with W3 presented separately in Table~\ref{tab:w3_detail}. This uncertainty reflects the precision of the ensemble mean. The \textbf{physical scatter} between regions is larger: the sample standard deviation of the 8 regional measurements is approximately 0.010 pc ($\sim$5\% of the mean), and the interquartile range is $0.203$--$0.217$ pc. Individual measurements span $0.188$--$0.218$ pc for the robust regions. After projection correction (factor of 1.27), the inferred 3D spacing is approximately 0.27 pc, or approximately 2.7 times the characteristic filament width.
...
\end{tabular}
\vspace{-0.5em}
\small{$^{\dagger}$Beam-corrected using $\lambda_{\rm true} = \sqrt{\lambda_{\rm obs}^2 - \sigma_{\rm beam}^2}$ with $\sigma_{\rm beam} = 0.035$ pc. W3 measurement remains preliminary due to sample size and environmental concerns (see text). The quadrature correction methodology for spacing measurements is approximate; we add an additional systematic uncertainty of $\pm$0.010 pc (5\%) for W3.}
\end{table}
...
Taurus & 0.205 $\pm$ 0.041 & No direct comparison & \textit{See Note} & \textcolor{green}{\textbullet} \\
W3     & 0.222 $\pm$ 0.038 & -- & \textbf{New measurement} & \textbf{First reported} \\
\bottomrule
\end{tabular}
\vspace{-0.5em}
\small{\textit{Note: Taurus has been analyzed in other HGBS-related works (e.g., Kirk et al. 2013 for TMC-1, a subregion within Taurus), but no direct core spacing measurement for the full Taurus HGBS region has been published. TMC1 is a separate high-density subregion analyzed independently in this work.}}
\end{table}

\textbf{California molecular cloud and systematic uncertainty consistency}: The study of \citet{Zhang2024} reports core spacings of approximately 0.15 pc in filaments \#8 and \#10 of the California cloud, corresponding to approximately $1.5\times W_{\rm fil}$. This is substantially smaller than the HGBS mean ($2.1\times$) and differs by approximately $6\sigma$ statistically. However, if we apply the same systematic uncertainty budget (40\%) to the California measurement, the uncertainty becomes approximately $\pm 0.06$ pc (40\% of 0.15 pc), giving $\lambda/W = 1.5 \pm 0.6$. In this case, the HGBS and California measurements overlap within combined systematic uncertainties. This suggests that environmental variation may be present, but the statistical significance of any difference is reduced when systematics are properly accounted for. Alternatively, genuine environmental differences between California (a low-mass, quiescent cloud) and HGBS regions (several with active star formation) could account for the different spacings. Without systematic uncertainty estimates for the California measurements, we cannot distinguish between these possibilities.

\textbf{Tension model prediction for California}: If the magnetic tension mechanism applies to California filaments, the observed spacing of $\lambda/W \approx 1.5$ requires $\beta \approx 0.3$ from equation~\ref{eq:tension_lambda}. The required field strength ($B \approx 15$--$20~\mu$G at filament densities) is physically plausible but represents stronger magnetisation than the $\beta \sim 1$--$3$ range inferred for HGBS filaments. Without direct field measurements in California filaments, this remains speculative.
...
\textbf{Validity of the $F(kR) \approx$ constant approximation}: For typical filament parameters ($R \sim 0.05$ pc, $\lambda \sim 0.2$ pc), we have $kR = 2\pi R/\lambda \sim 1.6$. This is not strictly in the $kR \ll 1$ regime. Numerical evaluation of the cylindrical gravitational potential function $F(kR)$ from \citet{Nakamura1993} shows that it varies by approximately 15\% over the range $0 < kR < 2$, with the variation becoming more rapid for $kR > 2$. A 15\% error in $F(kR)$ propagates to approximately a 7--8\% error in the predicted $\lambda/W$. This is comparable to but smaller than the systematic uncertainty in the observations ($\sim$40\%). We therefore proceed with the constant-$F$ approximation while noting this limitation. More precise calculations would require numerical solution of the full dispersion relation without this approximation.
...
\textbf{Validity of the perturbative approximation}: Equation~\ref{eq:tension_lambda} is derived under the assumption that magnetic effects are a perturbation to the hydrodynamic case, requiring $v_{A,\parallel} \lesssim c_s$ (i.e., $\beta \gtrsim 0.5$). For $\beta = 0.5$ ($v_{A,\parallel}/c_s = \sqrt{2} \approx 1.41$), the perturbative assumption is marginally violated. We have compared equation~\ref{eq:tension_lambda} against numerical solutions of the full dispersion relation (equation~10) for the $\beta$ values of interest. At $\beta = 0.5$, the perturbative result underestimates $\lambda/W$ by approximately 10\% compared to the full numerical solution. At $\beta = 1.0$, the error is approximately 5\%. These errors are comparable to the approximation error from the $F(kR) \approx$ constant assumption and are smaller than the observational systematic uncertainties. We therefore proceed with the perturbative approximation while noting these quantitative limitations.

\textbf{Physical interpretation}: The magnetic field acts like an elastic band. When the filament fragments, it must bend the field lines. Tension resists bending on \textit{large} scales but permits it on \textit{small} scales, effectively shifting the most unstable wavelength to \textit{shorter} wavelengths. In a 3D cylindrical geometry, this preferentially stabilises long-wavelength axial modes while allowing shorter-wavelength fragmentation to proceed. The physics is analogous qualitatively to a vibrating string: a tighter string vibrates at higher frequencies (shorter wavelengths), though the filament geometry adds important differences from the 1D string case.

\textbf{Plausible $\beta$ range and Zeeman extrapolation (with corrected calculations)}: For typical filament conditions ($n({\rm H_2}) \approx 10^4$ cm$^{-3}$, $T \approx 10$ K), the thermal pressure is $P_{\rm th} = n k_B T \approx (10^4~{\rm cm}^{-3})(1.38\times10^{-16}~{\rm erg~K}^{-1})(10~{\rm K}) \approx 1.4\times10^{-11}$ dyne cm$^{-2}$, assuming $n({\rm H}_2) \approx n_{\rm total}$ and including mean molecular weight corrections. Zeeman measurements in molecular cloud cores give magnetic field strengths in the range $B \approx 10$--$20$ $\mu$G at densities $n \sim 10^5$--$10^6$ cm$^{-3}$ \citep{Crutcher2012}, with some measurements up to $B \sim 30$--$50~\mu$G in the densest cores.
...
The corrected calculations give $\beta \approx 0.6$--$40$ for the extrapolated field strengths, encompassing both regimes where magnetic tension would have negligible effect ($\beta \gg 3$, predicting $\lambda/W \approx 4$, i.e., no modification from IM92) and regimes where tension could modify the spacing ($\beta \sim 0.6$--$2$, predicting $\lambda/W \approx 2.2$--$2.8$). Direct field measurements in filament interiors are needed to test which regime is realised in HGBS filaments.
...
\end{equation}
which makes the discrepancy \emph{worse} rather than better. For $\beta = 1$ ($v_A/c_s = 1$), this gives $\lambda/W \approx 5.7$; for $\beta = 0.3$ ($v_A/c_s \approx 1.8$), this gives $\lambda/W \approx 8.5$. The range of 5.7--$8.5\times$ corresponds to the quoted 7--$18\times$ range (which includes higher field strengths). These values all make the discrepancy with observations ($\lambda/W \approx 2.1$) significantly worse than the classical IM92 prediction.
...
\end{tabular}
\vspace{-0.5em}
\small{$^{\dagger}$Provisional: Results at $\beta = 0.1$ may be affected by incomplete saturation; see text. \textit{Note: Observations are shown as both projected 2D values (for consistency with published HGBS analyses) and deprojected 3D values (for like-for-like comparison with 3D theory).}}
\end{table}
...
\textbf{Constraints on field strength from $\beta = 0.1$ simulations (PROVISIONAL)}: The $\beta = 0.1$ simulations predict $\lambda/W \approx 0.87$ (sub-width fragmentation), which is inconsistent with the observed spacing. \textbf{However}, given the possible incomplete saturation at $\beta = 0.1$ (evidenced by extreme anisotropy ratios of 77--1345), this constraint is provisional. If high-resolution polarimetric measurements in HGBS filament interiors yield $\beta \gg 3$, the tension model would be disfavoured because the required field would be too weak to modify the fragmentation scale significantly. But the $\beta = 0.1$ results alone cannot definitively rule out low-$\beta$ regimes without confirmation of simulation convergence.
...
\end{itemize}
This intermediate value suggests that \textbf{real filaments are moderately supercritical}, with $f \approx 2$--$3$. \textbf{However, this is not directly demonstrated by our simulations.} This is an inference from interpolation between two limits. To definitively establish whether magnetic tension contributes at $f \approx 2$--$3$, simulations at these supercriticality values are required. In the absence of such simulations, the central quantitative claim that magnetic tension predicts $\lambda/W \approx 2.2$ for $\beta \sim 1$ rests entirely on the analytic perturbation theory (equation~13), not on any self-gravitating simulation. The self-gravitating simulations provide only a lower limit ($f = 6.6$ gives $\lambda/W \approx 0.94$) and the IM92 theory provides an upper limit ($f \approx 1$ gives $\lambda/W \approx 4$). The tension mechanism's prediction for the relevant regime ($f \approx 2$--$3$) remains untested by self-gravitating MHD simulations.
...
For $N_{\rm fiber}$ independent fibers with random uniform offsets, the statistical origin of the $1/\sqrt{N_{\rm fiber}}$ factor can be understood as follows: when measuring spacings between randomly selected cores without regard to fiber membership, we effectively sample from a superposition of $N_{\rm fiber}$ uniformly-spaced sequences that are randomly shifted relative to each other. The root-mean-square spacing of this superposition is compressed by approximately $1/\sqrt{N_{\rm fiber}}$ relative to the intrinsic spacing (this follows from the statistical properties of superposed periodic sequences with random phases; see, e.g.,~\citet{Cheng2014} for similar analyses in other contexts). For random offsets uniformly distributed over one wavelength, the apparent spacing is approximately:
\begin{equation}
...
For $N_{\rm fiber} = 3$, this gives $\lambda/W \approx 2.3$; for $N_{\rm fiber} = 4$, this gives $\lambda/W \approx 2.0$. The observed value of $2.1$ is consistent with both $N_{\rm fiber} = 3$ and $N_{\rm fiber} = 4$. We emphasise $N_{\rm fiber} \approx 3$ in the text because this is the number of fibers identified by \citet{Yang2024} in their analysis of Orion B, but the model is not sensitive to this choice: both $N_{\rm fiber} = 3$ and $N_{\rm fiber} = 4$ produce values consistent with observations.

This calculation assumes: (1) random uniform offsets between fibers; (2) independent fibers with no interaction; (3) no projection effects beyond the inter-fiber spacing compression. \textbf{Caveat}: This model is illustrative only. Real filaments have more complex geometry, fibers may interact gravitationally, and the "correct" value of $N_{\rm fiber}$ is not observationally constrained for HGBS filaments outside Orion B. The agreement with observations ($\lambda/W \approx 2.1$) should not be over-interpreted given these uncertainties.
...
From Planck Collaboration (2016), approximately 70\% of dense filaments are within $20^\circ$ of perpendicular to the local magnetic field (their Figure 5). This means that approximately 30\% are either parallel or have an intermediate orientation ($>20^\circ$ from perpendicular). The relative orientation histogram in Planck Figure 5 shows that approximately 10\% of dense filaments are within $20^\circ$ of parallel (longitudinal geometry).

Under the conservative assumption that HGBS filaments follow Planck statistics, we obtain:
\begin{equation}
...
For a filament with plasma $\beta \sim 1$--$3$ (consistent with the corrected Zeeman measurements extrapolated to filament densities), the tension-modified scale is:
\begin{equation}
\lambda_{\rm frag} \approx 4W \times \frac{c_s}{\sqrt{c_s^2 + v_A^2}} \approx 2.2W\text{--}2.8W
\label{eq:tension2}
\end{equation}

This provides a \textbf{consistency check}—grounded in turbulent field properties derived from numerical simulations—that the observed fragmentation scale can be accommodated within the tension framework for physically reasonable field strengths and field geometries (longitudinal). However, we emphasise that this is a consistency check, not a predictive test.

\textbf{Constraints on field strength (provisional)}: The $\beta = 0.1$ simulations (provisional due to possible incomplete saturation) predict $\lambda/W \approx 0.87$ (sub-width fragmentation), which is inconsistent with the observed spacing. If these simulations have reached saturation, this would constrain the typical field strength in HGBS filaments to $\beta \sim 0.5$--$1.0, placing an upper bound on field strength. However, given the possible incomplete saturation at $\beta = 0.1$, this constraint is provisional. If high-resolution polarimetric measurements in HGBS filament interiors yield $\beta \gg 3$, the tension model would be disfavoured because the required field would be too weak to modify the fragmentation scale significantly.
...
10. \textbf{Approximation errors quantified}: F(kR) and perturbative approximation errors explicitly estimated (7--10\% and 5--10\% respectively)
...
2. \textbf{Gap in self-gravitating simulations at $f \approx 2$--$3$}: Our highly supercritical simulations ($f = 6.6$) provide boundary conditions only and do not test the relevant regime. The tension mechanism's central quantitative claim rests entirely on analytic theory, not on self-gravitating simulations.
...
4. \textbf{Parameter space demonstration not test}: The 3,240 simulations show that some parameter combinations can produce the observed spacing, but we do not establish that these specific combinations are realized in HGBS filaments. Only 3\% of observationally-constrained combinations fall in the observed range, indicating poor predictive power.
...
8. \textbf{Approximation errors}: The F(kR) $\approx$ constant and perturbative approximations introduce 7--10\% uncertainties in the predicted $\lambda/W$, comparable to but smaller than observational systematics
...
\begin{enumerate}
    ...
    \item \textbf{Hierarchical fragmentation provides the most parsimonious explanation}: The observation by \citet{Yang2024} that fiber-to-core spacing in Orion B recovers the classical $4\times$ prediction suggests that the apparent sub-Jeans spacing at the filament level may reflect projection/averaging effects from measuring across hierarchical structures rather than a true modified fragmentation wavelength. A simple projection bias model with $N_{\rm fiber} \approx 3$--$4$ predicts $\lambda/W \approx 2.0$--$2.3$, consistent with observations. This interpretation is directly supported by observations and does not require modified physics. Fiber-resolved core spacing analysis in HGBS filaments is the single most critical test to distinguish this from the modified fragmentation scale interpretation.

    \item \textbf{Magnetic tension provides a consistency check, not a prediction}: For a filament threaded by a longitudinal magnetic field of equipartition strength ($\beta \sim 1$--$3$ based on corrected Zeeman extrapolation), magnetic tension reduces the fragmentation scale from the classical $4\times$ prediction to $\lambda/W = 2.2$--$2.8$ (projected) or $\lambda/W = 2.9$--$3.5$ (deprojected), which overlaps with observations at the projected value but underestimates the deprojected value by approximately 15--20\%. However, this is a necessary but not sufficient consistency test, not a predictive test: for any observed $\lambda/W$, a $\beta$ can be found to satisfy the equation. The model would be disfavoured if high-resolution polarimetric measurements in HGBS filament interiors yield $\beta \gg 3$ (fields too weak to modify the fragmentation scale). \textbf{The central quantitative claim of the tension mechanism rests entirely on analytic perturbation theory (equation~13), not on self-gravitating MHD simulations.}

    \item \textbf{Magnetic tension faces substantial geometric challenges}: The entire magnetic tension analysis rests on the assumption that magnetic fields threading filaments are aligned with the filament axis (longitudinal geometry). However, Planck Collaboration (2016) found that dense star-forming filaments tend to be perpendicular to the mean magnetic field, and FIELDMAPS finds predominantly perpendicular fields on parsec scales where HGBS filaments are measured. Under conservative assumptions based on Planck Figure 5, we estimate a prior probability of only approximately 10\% that a randomly selected HGBS filament has predominantly longitudinal internal field geometry. This geometric challenge—combined with the observational evidence for hierarchical fragmentation—constitutes the most significant limitation of the tension mechanism.

    \item \textbf{Self-gravitating MHD simulations provide boundary conditions only and do not test the relevant regime}: Simulations of a highly supercritical filament ($f = 6.6$) with Athena++ show that core spacing is $\lambda/W \approx 0.94$ and is \textbf{independent of magnetic field strength} ($\beta = 0.5$--$2.0$). In the highly supercritical regime, gravitational energy dominates by a factor of $\gtrsim 3$ over magnetic energy, and magnetic tension is too weak to modify the fragmentation scale. The observed spacing ($\lambda/W \approx 2.1$) lies between the highly supercritical limit and the near-critical IM92 prediction ($\lambda/W \approx 4$), suggesting moderate supercriticality ($f \approx 2$--$3$). \textbf{However, this is not directly demonstrated by our simulations.} No self-gravitating simulation exists in the relevant regime ($f \approx 2$--$3$). The tension mechanism's prediction for this regime rests entirely on analytic theory, not on simulations. Simulations at $f = 2$--$3$ are urgently needed to test whether magnetic tension contributes in the moderately supercritical regime.

    \item \textbf{Field strength constraints from simulations are provisional}: The $\beta = 0.1$ simulations predict $\lambda/W \approx 0.87$ (sub-width fragmentation), which is inconsistent with the observed spacing. \textbf{However}, given the possible incomplete saturation at $\beta = 0.1$ (evidenced by extreme anisotropy ratios of 77--1345 that may indicate non-equilibrium), this constraint is provisional. The claim that $\beta \ll 0.5$ is ruled out by simulations cannot be made definitively without confirmation that the $\beta = 0.1$ runs have reached statistical equilibrium. If high-resolution polarimetric measurements in HGBS filament interiors yield $\beta \gg 3$, the tension model would be disfavoured because the required field would be too weak to modify the fragmentation scale significantly.
