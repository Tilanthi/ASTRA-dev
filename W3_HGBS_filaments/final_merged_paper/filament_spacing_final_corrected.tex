...
We examine whether magnetic tension along filament axes provides a consistent explanation for this factor-of-two discrepancy. For a filament threaded by a longitudinal magnetic field, linear perturbation analysis yields a tension-modified fragmentation wavelength of $\lambda/W = 4c_s/\sqrt{c_s^2 + v_{A,\parallel}^2} = 4/\sqrt{1 + \beta^{-1}}$, where $\beta$ is the plasma beta. \textbf{This is a necessary but not sufficient consistency test}: for any observed $\lambda/W$, a $\beta$ can be found to satisfy the equation. The model can be falsified by $\beta$ measurements outside physically plausible ranges ($\beta < 0.1$ or $\beta > 3$), or by direct field geometry measurements showing predominantly perpendicular alignment. A predictive test would require independent measurement of $\beta$ in the same filaments where $\lambda/W$ is measured, which is not currently available. For $\beta \in [0.5, 1.0]$, the model gives $\lambda/W \in [1.8, 2.3]$, which overlaps with our observational measurements.
...
\textbf{Critical caveat}: The entire magnetic tension analysis rests on the assumption that magnetic fields threading filaments are aligned with the filament axis (longitudinal geometry). However, Planck Collaboration (2016) found that dense star-forming filaments tend to be perpendicular to the mean magnetic field. Under the conservative assumption that HGBS filaments follow Planck statistics, we estimate a prior probability of only approximately 10--15\% that a randomly selected HGBS filament has predominantly longitudinal internal field geometry. This geometric challenge—combined with the fact that fiber-to-core spacing in Orion B recovers the classical $4\times$ prediction \citep{Yang2024}—constitutes the most significant limitation of the tension mechanism. Recent high-resolution polarimetric observations (SOFIA/HAWC+, ALMA) reveal complex scale-dependent field geometries within individual filaments, but these have been detected in only a small number of systems and their prevalence in HGBS filaments is unknown.
...
Together, these analyses indicate that magnetic tension is \textbf{a hypothesis that remains consistent with observations}, but which faces substantial challenges from field geometry measurements and from the hierarchical fragmentation alternative. Definitive tests require direct measurement of magnetic field geometry and strength in HGBS filaments, as well as fiber-resolved core spacing analysis to test the hierarchical interpretation.
\end{abstract}
...
\textbf{Beam dilution correction}: For a Gaussian beam, the observed spacing $\lambda_{\rm obs}$ is related to the true spacing $\lambda_{\rm true}$ by:
\begin{equation}
\lambda_{\rm obs} \approx \sqrt{\lambda_{\rm true}^2 + \sigma_{\rm beam}^2}
\end{equation}
where $\sigma_{\rm beam} = 0.035$ pc is the beam width. Applying this correction to the W3 measurement gives $\lambda_{\rm true} \approx \sqrt{0.225^2 - 0.035^2} = 0.222$ pc, a correction of approximately 1.3\%. This is smaller than the 10--20\% estimated in earlier work because the beam dilution effect is modest when the beam size is a small fraction of the spacing. We therefore flag W3 as \textbf{Preliminary} primarily due to the H\,{\scriptsize II} masking and sample size concerns, with beam dilution as a secondary concern.
...
\subsection{Approximately Uniform Core Spacing Across 8 High-Confidence HGBS Regions}
...
Given these uncertainties, we temper our claims and state that the data are consistent with an approximately uniform fragmentation scale within current uncertainties, but we cannot rule out environmental variation at the 10\% level. The word "universal" in the abstract has been replaced with "approximately uniform" to reflect this limitation.
...
\textbf{Regime of validity}: The perturbative approximation requires $v_{A,\parallel} \lesssim c_s$, which corresponds to $\beta \gtrsim 0.5$. For $\beta < 0.5$, the full dispersion relation must be solved numerically. The $\beta \in [0.5, 1.0]$ range we invoke falls within the regime of validity of the perturbative approximation. The approximation $F(kR) \approx$ constant is justified for $kR \ll 1$ because the cylindrical gravitational potential varies logarithmically with radius, making the geometric factor slowly varying in the long-wavelength limit.
...
For $B_{\rm core} = 10$--$20~\mu$G and $\kappa = 0.5$--$0.65$, this yields $B_{\rm fil} \approx 3$--$10~\mu$G at filament densities, corresponding to $\beta \approx 0.1$--$1.0. This extrapolation shows that the Zeeman data encompass both regimes that would falsify the tension model ($\beta \approx 0.1$, predicting $\lambda/W \approx 0.87$, inconsistent with observations) and regimes consistent with it ($\beta \approx 1$, predicting $\lambda/W \approx 2.2$). The extrapolation therefore does not uniquely select $\beta \sim 1$ as the only plausible value — it merely demonstrates that $\beta \sim 1$ is within the range of possibilities permitted by existing Zeeman constraints. Direct field measurements in filament interiors are needed to test whether $\beta \gtrsim 0.5$ or $\beta \lesssim 0.3$ in the regions where fragmentation actually occurs.
...
This calculation assumes: (1) random uniform offsets between fibers; (2) independent fibers with no interaction; (3) no projection effects beyond the inter-fiber spacing compression. Despite these simplifying assumptions, the order-of-magnitude agreement with observations ($\lambda/W \approx 2.1$) is striking.
...
For a filament with plasma $\beta \sim 1$ (consistent with the upper end of Zeeman measurements extrapolated to filament densities), the tension-modified scale is:
\begin{equation}
...
\textbf{Constraints on field strength}: The $\beta = 0.1$ simulations predict $\lambda/W \approx 0.87$, which is inconsistent with the observed spacing. This constrains the typical field strength in HGBS filaments to $\beta \sim 0.5$--$1.0, placing an upper bound on field strength. If high-resolution polarimetric measurements in HGBS filament interiors yield $\beta \gg 1$, the tension model would be disfavoured.
...
\begin{enumerate}
    ...
    \item \textbf{Magnetic field strength measurements}: DCF method applied to new polarimetric data can quantify whether HGBS filaments are in the transcritical regime (as suggested by our intermediate $\lambda/W \approx 2.1$ spacing) or in a different regime. Field strengths of approximately 100--200~$\mu$G would be consistent with $\beta \sim 1$, while $\beta \gg 1$ would disfavour the tension model.
...
\begin{enumerate}
    \item \textbf{Approximately uniform core spacing}: $0.211 \pm 0.007$ pc (statistical) across 8 high-confidence HGBS regions (5,069 cores), corresponding to $2.11\times$ the characteristic filament width. Including the preliminary W3 measurement (beam-corrected to 0.222 pc) gives $0.212 \pm 0.007$ pc across all 9 regions. After projection correction, the inferred 3D spacing is approximately 0.27 pc (approximately 2.7$\times W_{\rm fil}$). The data are consistent with an approximately uniform fragmentation scale given current uncertainties, but modest environmental variation at the 10\% level cannot be ruled out.
...
\textbf{Software}: This work used Python 3.10 \citep{Python2023} with NumPy 1.23.4 \citep{Harris2020}, SciPy 1.9.3 \citep{Virtanen2020}, and Matplotlib 3.6.2 \citep{Hunter2007}. MHD simulations were performed with Athena++ 21.0 \citep{Stone2020}. Core extraction used DisPerSE \citep{Sousbie2011}.
