#!/usr/bin/env python3
"""
Forward modelling of PM-NN discrepancy in multi-filament systems (FIXED VERSION).
...
class MultiFilamentForwardModel:
    ...
    def __init__(self, L=5.0, W=0.1, lambda_true=0.20, n_filaments=3,
        ...
        # Storage
        self.cores = None
        self.true_filament_ids = None  # Track true filament assignment
    ...
    def generate_filament_system(self):
        ...
        """
        cores = []
        filament_ids = []  # Track which filament each core belongs to
        ...
        for i in range(self.n_filaments):
            ...
            for j in range(self.n_beads_per_filament):
                ...
                if 0 <= x <= self.L:
                    ...
                    cores.append([x + x_scatter, y_center + y_scatter])
                    filament_ids.append(i)

        # Convert to numpy array
        self.cores = np.array(cores)
        self.true_filament_ids = np.array(filament_ids)
        return self.cores
    ...
    def compute_nn_spacing_filament_projected(self):
        ...
        FIXED VERSION: Use true filament assignments instead of clustering.
        """
        if self.cores is None:
            self.generate_filament_system()
        ...
        # For each filament, compute NN spacings using true assignment
        for filament_id in np.unique(self.true_filament_ids):
            # Get cores belonging to this filament
            filament_mask = self.true_filament_ids == filament_id
            filament_cores = self.cores[filament_mask]

            if len(filament_cores) < 2:
                continue

            # Project along filament axis (x-axis for horizontal filaments)
            # Since filaments are horizontal, the x-coordinate IS the projection
            projections = filament_cores[:, 0]  # x-coordinates
...
def run_parameter_sweep():
    ...
    - n_realizations: 50 per parameter point

    Total: 6 * 4 * 4 * 3 * 50 = 14,400 realizations
    """
    print("=" * 80)
    print("FORWARD MODELLING: PM-NN DISCREPANCY IN MULTI-FILAMENT SYSTEMS (FIXED VERSION)")
    print("=" * 80)
    ...
    # Save results
    results_file = 'forward_model_pm_nn_results_fixed.json'
...
def analyze_results(results):
    ...
    """
    print("\n" + "=" * 80)
    print("ANALYZING RESULTS (FIXED VERSION)")
    print("=" * 80)
...
def create_analysis_figures(df):
    ...
    ax.set_title('A. PM/NN Ratio vs. Number of Filaments (FIXED)', fontsize=13, fontweight='bold')
    ...
    ax.set_xlabel(r'Inter-Filament Spacing / $\lambda_{true}$', fontsize=12)
    ax.set_ylabel('PM / NN Ratio', fontsize=12)
    ax.set_title('B. PM/NN Ratio vs. Inter-Filament Spacing (FIXED)', fontsize=13, fontweight='bold')
    ax.legend(fontsize=10)
    ax.grid(True, alpha=0.3)
    ax.set_ylim([0.8, 2.5])
    ax.set_xscale('log')
    ...
    ax.axhline(y=1.45, color='red', linestyle='--', linewidth=2, label='Observed (1.45)')
    ax.axhline(y=1.0, color='gray', linestyle=':', linewidth=1.5, label='Single filament (1.0)')
    ax.set_xlabel(r'Position Scatter / $\lambda_{true}$', fontsize=12)
    ax.set_ylabel('PM / NN Ratio', fontsize=12)
    ax.set_title('C. PM/NN Ratio vs. Position Scatter (FIXED)', fontsize=13, fontweight='bold')
    ax.legend(fontsize=10)
    ax.grid(True, alpha=0.3)
    ax.set_ylim([0.8, 2.5])
    ...
    ax.set_xlabel('Number of Filaments', fontsize=12)
    ax.set_ylabel(r'Inter-Filament Spacing / $\lambda_{true}$', fontsize=12)
    ax.set_title('D. PM/NN Ratio Phase Diagram (FIXED)', fontsize=13, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'forward_model_pm_nn_analysis_fixed.pdf'), dpi=300, bbox_inches='tight')
    plt.savefig(os.path.join(output_dir, 'forward_model_pm_nn_analysis_fixed.png'), dpi=300, bbox_inches='tight')
    print(f"\nFigure saved: {output_dir}/forward_model_pm_nn_analysis_fixed.pdf")
    ...
    ax.set_title('PM Bias vs. Number of Filaments (FIXED)', fontsize=13, fontweight='bold')
    ax.grid(True, alpha=0.3)
    ...
    ax.set_title('NN Bias vs. Number of Filaments (FIXED)', fontsize=13, fontweight='bold')
    ax.grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'forward_model_bias_analysis_fixed.pdf'), dpi=300, bbox_inches='tight')
    print(f"Figure saved: {output_dir}/forward_model_bias_analysis_fixed.pdf")
...
def generate_summary_report(df):
    ...
    """
    report = []
    report.append("# Forward Modelling: PM-NN Discrepancy Analysis (FIXED VERSION)")
    ...
    if abs(single_fil['pm_nn_ratio'].mean() - 1.0) < 0.1:
        report.append(f"\n✅ **Single-filament control is correct** (PM/NN ≈ 1.0).")
    else:
        report.append(f"\n⚠️ **Single-filament control shows PM/NN = {single_fil['pm_nn_ratio'].mean():.3f}, not 1.0 as expected.")
    ...
    # Save report
    with open('FORWARD_MODEL_SUMMARY_FIXED.md', 'w') as f:
        f.write(report_text)

    print(report_text)
    print(f"\nReport saved to: FORWARD_MODEL_SUMMARY_FIXED.md")
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 80)
    print("FORWARD MODELLING COMPLETE (FIXED VERSION)")
    print("=" * 80)
