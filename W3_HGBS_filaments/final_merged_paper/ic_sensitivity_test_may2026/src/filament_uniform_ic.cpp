// PROBLEM GENERATOR: Uniform Density IC for IC Sensitivity Test
// Athena++ problem generator for self-gravitating magnetized filaments
// Uses uniform density distribution: rho(x,y) = rho0 = constant
//
// Parameter space: f = 1.0-1.3, beta = 0.3-1.0, M = 1.0-2.0
// Purpose: Test IC sensitivity in near-critical regime
//
// NOTE: This is the ONLY difference between King and Uniform IC generators.
//       All other parameters and perturbations are identical.
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  // Uniform density (for near-critical filaments, rho0 ≈ f)
  rho0 = f_line;
...
void Fluid::InitFluid(ParameterInput *pin) {
  // Initialize uniform density filament with PRR perturbations
  ...
  for (int k = pmy_block->ks; k <= pmy_block->ke; ++k) {
    for (int j = pmy_block->js; j <= pmy_block->je; ++j) {
      for (int i = pmy_block->is; i <= pmy_block->ie; ++i) {
        ...
        // UNIFORM DENSITY (key difference from King IC)
        Real rho = rho0;
        ...
        // NOTE: Identical to King IC implementation
        Real dvx = 0.0;
